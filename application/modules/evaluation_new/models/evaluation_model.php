  ...
  class evaluation_model extends CI_Model {
       ...
       public function __construct() {
          ...
          //   $this->tbl_valuation_status = TABLE_PREFIX . 'valuation_status_1';
           ...
           // $this->tbl_purchase_check_list = TABLE_PREFIX . 'purchase_check_list_1';
           $this->tbl_purchase_check_list = TABLE_PREFIX . 'purchase_check_list';
          //  $this->tbl_purchase_check_list_details = TABLE_PREFIX . 'purchase_check_list_details_1';
...
//debug( $totalRecordwithFilter);
            ...
            if (isset($filterDatas['status']) && ($filterDatas['status'] >= 0)) {
                 if($filterDatas['status']==39){//stock veh 
                    $this->db->where('(' . $this->tbl_valuation . ".val_status=39  OR " . $this->tbl_valuation . ".val_status =43 OR " . $this->tbl_valuation . ".val_status =66 )");
            ...
            $this->db->where($this->tbl_valuation . '.val_parent_id', NULL);
            if ($rowperpage > 0) {
       ...
       function getEvaluation_jk($id = '', $status = '') {
          $this->load->model('enquiry/enquiry_model', 'enquiry');
          if (!empty($id)) {
               $EvaluationVehicle = $this->db->select($this->tbl_valuation . '.*,' . $this->tbl_model . '.*,' .
                                       $this->tbl_brand . '.*,' . $this->tbl_variant . '.*,' . $this->tbl_users . '.*,' . $this->tbl_showroom . '.*')
       ...
       function newEvaluation($data) {
            if (!empty($data)) {
                 ...
                 isset($data['val_wrnty_nxt_ser_date']) ? $data['val_wrnty_nxt_ser_date'] = date('Y-m-d', strtotime($data['val_wrnty_nxt_ser_date'])) : '';
                  isset($data['val_next_serv_date']) ? $data['val_next_serv_date'] = date('Y-m-d', strtotime($data['val_next_serv_date'])) : '';
       ...
       function getOwnParkAndSaleCars_bk() {
            return $this->db->select($this->tbl_valuation . '.*,' . $this->tbl_model . '.*,' . $this->tbl_brand . '.*,' .
                            ...
                            ->join($this->tbl_showroom, $this->tbl_showroom . '.shr_id = ' . $this->tbl_valuation . '.val_showroom', 'left')
                            ->join($this->tbl_enquiry, $this->tbl_enquiry . '.enq_id = ' . $this->tbl_valuation . '.val_enquiry_id', 'left')
                            //->where_in($this->tbl_valuation . '.val_type', array(1, 2, 3,4))
                            //->where($this->tbl_valuation . '.val_status', 39)/* Stock vehicle */
                            ->where('val_refurb_status != 66')
                            ->where('(' . $this->tbl_valuation . ".val_status=39  OR " . $this->tbl_valuation . ".val_status =43)")
                                                       ->get($this->tbl_valuation)->result_array();
       }
   ...
   ->join($this->tbl_users_groups, $this->tbl_users_groups . '.user_id = ' . $this->tbl_users . '.usr_id')
   ->join($this->tbl_groups, $this->tbl_groups . '.id = ' . $this->tbl_users_groups . '.group_id')
   ->where('('.$this->tbl_groups . ".id IN (13,8,6,3) AND " . $this->tbl_users . ".usr_departments IN (7,8,1,2))")
...
//           return $this->db->select($this->tbl_showroom . '.shr_location,')->where('shr_division',$division)->result_array();
     ...
     function getRefurbStatus() {
                 ...
                 return $this->db->where('sts_category', 'refurb')->get($this->tbl_statuses)->result_array(); 

     }
     function updateRefstatus($data) {
          if($data['val_refurb_status']){
                           // $data['val_status']=$data['val_refurb_status'];
               // debug($data);
...
//            }
     ...
     function getAllAPMASM() {
          return $this->db->select($this->tbl_users . '.usr_id AS col_id, ' . $this->tbl_users . '.usr_first_name AS col_title, ' .
                            ...
                            'usr_designation_new = 22 OR usr_designation_new = 24 OR usr_designation_new = 40 OR usr_designation_new = 60' . "))")
     ...
     function getDelco() {
          ...
          return $this->db->select('usr_id, usr_username, usr_first_name, usr_last_name, shr_location')
                          ->join($this->tbl_showroom, $this->tbl_showroom . '.shr_id = ' . $this->tbl_users . '.usr_showroom', 'left')
                          ->where(array($this->tbl_users . '.usr_active' => 1, $this->tbl_users . '.usr_resigned' => 0))
                          ->where_in('usr_designation_new', array(21, 15))->get($this->tbl_users)->result_array();
     }
