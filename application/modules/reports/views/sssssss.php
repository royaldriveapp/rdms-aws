...
class reports_model extends CI_Model
{
     ...
     function searchEnquiryByDateShowroomSe($limit = 0, $page = 0, $data)
     {
          ...
          if (isset($data['exp']) && !empty($data['exp'])) {
               $selectArray = array(
                    $this->tbl_enquiry . '.enq_number AS Enquiry-Number',
                    $this->tbl_enquiry . '.enq_budget AS Budget',
                    $this->tbl_enquiry . '.enq_entry_date AS Enq-Date',
                    $this->tbl_enquiry . '.enq_cus_name AS Customer',
                    $this->tbl_enquiry . '.enq_cus_mobile AS Customer-Number',
                    $this->tbl_enquiry . '.enq_cus_address AS Address',
                    $this->tbl_enquiry . '.enq_cus_loan_perc AS Loan-Per',
                    $this->tbl_enquiry . '.enq_cus_loan_amount AS Loan-AMT',
                    $this->tbl_enquiry . '.enq_cus_loan_emi AS AS Loan-EMI',
                    $this->tbl_enquiry . '.enq_cus_loan_period AS Loan-Period',
                    $this->tbl_enquiry . '.enq_cus_remarks AS Customer-Remarks',
                    "(CASE WHEN " . $this->tbl_valuation . ".enq_cus_when_buy = 1 THEN 'Hot +'
                    WHEN " . $this->tbl_valuation . ".enq_cus_when_buy = 2 THEN 'Hot'
                    WHEN " . $this->tbl_valuation . ".enq_cus_when_buy = 3 THEN 'Warm'
                    WHEN " . $this->tbl_valuation . ".enq_cus_when_buy = 4 THEN 'Cold' END) AS Customer-Status",
                    $this->tbl_enquiry . '.enq_next_foll_date AS Next-Followup-On',
                    $this->tbl_enquiry . '.enq_added_on AS Enquiry-Created-On',
                    $this->tbl_enquiry . '.enq_last_foll_cust_rmk Last-Customer-Remark',
                    $this->tbl_enquiry . '.enq_last_foll_date AS Last-Followup-On',
                    $this->tbl_enquiry . '.enq_home_visit_date AS Home-Visit-On',
                    $this->tbl_enquiry . '.enq_sm_rmk AS Last-SM-Remark',
                    $this->tbl_enquiry . '.enq_asm_rmk AS Last-ASM-Remark',
                    $this->tbl_enquiry . '.enq_mis_rmk AS Last-MIS-Remark',
                    "(CASE WHEN " . $this->tbl_valuation . ".enq_cus_status = 1 THEN 'Sales'
                    WHEN " . $this->tbl_valuation . ".enq_cus_status = 2 THEN 'Purchase'
                    WHEN " . $this->tbl_valuation . ".enq_cus_status = 3 THEN 'Exchange' END) AS Enquiry-Type",
                    $this->tbl_users . '.usr_first_name AS Present-Sales-Staff',
                    $this->tbl_showroom . '.shr_location AS Showroom',
                    $this->tbl_occupation . '.occ_name AS Profession',
                    $this->tbl_city . '.cit_name AS Cust-Location',
                    'teamLead.usr_username AS Team-Lead',
                    $this->tbl_district_statewise . '.std_district_name AS District',
                    'createdby.usr_first_name AS Created-By',
                    $this->tbl_divisions . '.div_name AS Division',
                    $this->tbl_statuses . '.sts_title AS Enq-Status',
                    $this->tbl_enquiry_meta . '.enqm_pur_veh AS Purchase-Vehicle',
                    $this->tbl_enquiry_meta . '.enqm_sls_veh AS Sales-Vehicle',
                    $this->tbl_valuation . '.val_status AS Evaluated',
                    $this->tbl_contact_mode . '.cmd_title AS Mod-Of-Contact'
               );
          } else {
               $selectArray = array(
                    $this->tbl_enquiry . '.enq_id',
...
WHERE   datediff(current_date,date(val_purchased_date)) $gl $no AND val_type=$type AND val_re_evaluated=0";
     ...
     function getAsmData($asm, $date = null)
     { //asm Or apm
          $today_date = date('Y-m-d');
          $first_day_of_month = date('Y-m-01');
          if ($date) {
