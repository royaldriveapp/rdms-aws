<?php
  ...
  class reports_model extends CI_Model {
       ...
       public function __construct() {
            ...
            $this->re_assign_reports = TABLE_PREFIX . 're_assign_reports';
       }

       function allenquiresAjax($postDatas, $getData) {
            ...
            $columnName = isset($postDatas['columns'][$columnIndex]['data']) ? $postDatas['columns'][$columnIndex]['data'] : 'enq_id'; // Column name 
            ...
            if ($searchValue != '') {
                 $this->db->where("(enq_cus_name LIKE '%" . $searchValue . "%' OR enq_cus_whatsapp LIKE '%" . $searchValue . "%' OR enq_cus_mobile LIKE '%" . $searchValue . "%')");
            }
            /* Filter data */
            if (isset($getData['cmbStatus']) && !empty($getData['cmbStatus'])) {
                 $this->db->where_in('enq_current_status', $getData['cmbStatus']);
                 //$this->db->where_in($this->tbl_vehicle_booking_master . '.vbk_status', $getData['cmbStatus']);
            }
            if (isset($getData['cmbShowroom']) && !empty($getData['cmbShowroom'])) {
                 $this->db->where_in('enq_showroom_id', $getData['cmbShowroom']);
            }
            if (isset($getData['cmbSalesExecutives']) && !empty($getData['cmbSalesExecutives'])) {
                 $this->db->where_in('enq_se_id', $getData['cmbSalesExecutives']);
            }
            /* Filter data */
            //$this->db->join($this->tbl_vehicle_booking_master, $this->tbl_vehicle_booking_master . '.vbk_enq_id = ' . $this->tbl_enquiry . '.enq_id', 'LEFT');
            $totalRecordwithFilter = $this->db->count_all_results($this->tbl_enquiry);
            ...
            $select = array(
                ...
                $this->tbl_vehicle_booking_master . '.vbk_vehicle_amt',
                $this->tbl_vehicle_booking_master . '.vbk_tcs',
                $this->tbl_vehicle_booking_master . '.vbk_ttl_sale_amt',
                $this->tbl_vehicle_booking_master . '.vbk_advance_amt',
                $this->tbl_vehicle_booking_master . '.vbk_added_on',
                $this->tbl_vehicle_booking_master . '.vbk_expect_delivery',
            ...
            $return['data'] = $this->db->select($select)
                            ...
                            ->join($this->tbl_contact_mode, $this->tbl_contact_mode . '.cmd_id = ' . $this->tbl_enquiry . '.enq_mode_enq', 'LEFT')
       ...
       function getInnactiveVehicles($status, $limit = 0, $page = 0, $filter = array(), $noLimit = true) {
            ...
            if ($this->uid != 100) {
                 if (check_permission('enquiry', 'showdroplostmydivision')) {
                      $this->db->where('shr_division', $this->div);
                 } else if (check_permission('enquiry', 'showdroppedenquiriesmyteam')) {
                      $this->db->where_in($this->tbl_enquiry . '.enq_se_id', $mystaffs);
                 ...
                 while (strtotime($dataFrom) <= strtotime($dataTo)) {
                      ...
                      if (check_permission('reports', 'dar_mystaff')) {
                           ...
                           if (check_permission('reports', 'dar_salesdept') && check_permission('reports', 'dar_purdept')) {
                                if (check_permission('reports', 'dar_mydivision')) {
                                     $this->db->where($this->tbl_users . '.usr_division', $this->div);
                                }
                                $staffs = $this->db->select($selectArray)
                                                ->join($this->tbl_showroom, $this->tbl_showroom . '.shr_id = ' . $this->tbl_users . '.usr_showroom', 'LEFT')
                                                ->join($this->tbl_designation, $this->tbl_designation . '.desig_id = ' . $this->tbl_users . '.usr_designation_new', 'LEFT')
                                ...
                                $staffs = $this->db->select($selectArray)
                                                ...
                                                ->join($this->tbl_designation, $this->tbl_designation . '.desig_id = ' . $this->tbl_users . '.usr_designation_new', 'LEFT')
                                                ->where($this->tbl_users . '.usr_resigned', 0)
                                                ->where('usr_active', 1)->where_in($this->tbl_users_groups . '.group_id', array(4, 7, 8, 9, 11, 13))->get($this->tbl_users)->result_array();
                           }
                           //   if($this->uid == 853) {
                           //      echo $this->db->last_query();
                           //      debug($staffs);
       ...
       function darStaffFilter() {
            ...
            if (check_permission('reports', 'dar_mystaff')) {
                 ...
                 if ($this->usr_grp == 'TL') {
                      ...
                      //                 $this->db->where($this->tbl_users . '.usr_showroom', $this->shrm);
                      return $this->db->select($this->tbl_users . '.usr_id, ' . $this->tbl_users . '.usr_showroom,' . $this->tbl_users . '.usr_username,' . $this->tbl_users_groups . '.*,' . $this->tbl_groups . '.*')
                                      ->join($this->tbl_users_groups, $this->tbl_users_groups . '.user_id = ' . $this->tbl_users . '.usr_id', 'LEFT')
                                      ->join($this->tbl_groups, $this->tbl_groups . '.id = ' . $this->tbl_users_groups . '.group_id', 'LEFT')
                                      //->where(array($this->tbl_users . '.usr_can_auto_assign' => 1))
                                      ->where_in($this->tbl_groups . '.grp_slug', array('SE', 'TC', 'EV', 'TL'))
                                      ->order_by($this->tbl_users . '.usr_first_name', 'ASC')->get($this->tbl_users)->result_array();
                 } else {
                      return $this->db->select($this->tbl_users . '.usr_id, ' . $this->tbl_users . '.usr_showroom,' . $this->tbl_users . '.usr_username,' . $this->tbl_users_groups . '.*,' . $this->tbl_groups . '.*')
                                      ->join($this->tbl_users_groups, $this->tbl_users_groups . '.user_id = ' . $this->tbl_users . '.usr_id')
       ...
       function searchInquiryByDefault($limit = 0, $page = 0, $data) {
            ...
            $mystaffs = array();
            $staffs = array();
            if (isset($data['asm']) && !empty($data['asm'])) {
                 $staffs = my_staff($this->uid);
                 array_push($staffs, $data['asm']);
            }
            ...
            } else if (check_permission('reports', 'showonlymydivision')) {
                 $this->db->where($this->tbl_showroom . '.shr_division', $this->div);
            } else if (check_permission('enquiry', 'showonlymystaff')) {
                 $this->db->where_in($this->tbl_enquiry . '.enq_se_id', $mystaffs);
            } else if ($this->usr_grp == 'MG' && $this->uid != 48 && $this->uid != 870 && $this->uid != 927 && $this->uid != 691 && $this->uid != 641 && $this->uid != 11111 && $this->uid != 11080) { // Manager
                 $this->db->where($this->tbl_enquiry . '.enq_showroom_id', $showroom);
            ...
            if (!empty($staffs)) {
                 $this->db->where_in($this->tbl_enquiry . '.enq_se_id', $staffs);
            }
            ...
            $this->db->join($this->tbl_showroom, $this->tbl_showroom . '.shr_id = ' . $this->tbl_enquiry . '.enq_showroom_id', 'LEFT');
            $return['count'] = $this->db->count_all_results($this->tbl_enquiry);
            ...
            if (is_roo_user() || check_permission('enquiry', 'showall')) { // Admin users 
                 ...
                 $this->db->where_in($this->tbl_enquiry . '.enq_se_id', $mystaffs);
            } else if ($this->usr_grp == 'MG' && $this->uid != 48 && $this->uid != 870 && $this->uid != 927 && $this->uid != 691 && $this->uid != 641 && $this->uid != 11111 && $this->uid != 11080) { // Manager Ponnu, Shiny, Remya, Ancy
                 $this->db->where($this->tbl_enquiry . '.enq_showroom_id', $showroom);
            } else if ($this->usr_grp == 'DE' || (check_permission('enquiry', 'showmyshowroom'))) { // Date entry operator
                 $this->db->where($this->tbl_enquiry . '.enq_showroom_id', $showroom);
            } else if ($this->usr_grp == 'SE' && empty($data['executive'])) { // Seles executives
       ...
       function searchInquiryByDefaultNoLimit($limit = 0, $page = 0, $data) {
            ...
            if (is_roo_user() || check_permission(
                            'enquiry',
                            'showall'
                    )) { // Admin users 
            } else if (check_permission('reports', 'showonlymydivision')) {
       ...
       function voxbayPunchReprt_ajax($postDatas) {
            ...
            $selectArray = array(
                ...
                $this->tbl_register_master . '.*',
                "DATE_FORMAT(" . $this->tbl_register_master . '.vreg_added_on, "%M %d %Y") AS vreg_added_on',
       ...
       function searchEnquiryByDateShowroomSe($limit = 0, $page = 0, $data) {
            ...
            if (isset($data['exp']) && !empty($data['exp'])) {
                 $selectArray = array(
                     $this->tbl_enquiry . '.enq_number AS Enquiry_Number',
                     $this->tbl_enquiry . '.enq_budget AS Budget',
                     $this->tbl_enquiry . '.enq_entry_date AS Enq_Date',
                     $this->tbl_enquiry . '.enq_cus_name AS Customer',
                     $this->tbl_enquiry . '.enq_cus_mobile AS Customer_Number',
                     $this->tbl_enquiry . '.enq_cus_address AS Address',
                     $this->tbl_enquiry . '.enq_cus_loan_perc AS Loan_Per',
                     $this->tbl_enquiry . '.enq_cus_loan_amount AS Loan_AMT',
                     $this->tbl_enquiry . '.enq_cus_loan_emi AS Loan_EMI',
                     $this->tbl_enquiry . '.enq_cus_loan_period AS Loan_Period',
                     $this->tbl_enquiry . '.enq_cus_remarks AS Customer_Remarks',
                     "(CASE WHEN " . $this->tbl_enquiry . ".enq_cus_when_buy = 1 THEN 'Hot +'
                    WHEN " . $this->tbl_enquiry . ".enq_cus_when_buy = 2 THEN 'Hot'
                    WHEN " . $this->tbl_enquiry . ".enq_cus_when_buy = 3 THEN 'Warm'
                    WHEN " . $this->tbl_enquiry . ".enq_cus_when_buy = 4 THEN 'Cold' END) AS Customer_Status",
                     $this->tbl_enquiry . '.enq_next_foll_date AS Next_Followup_On',
                     $this->tbl_enquiry . '.enq_added_on AS Enquiry_Created_On',
                     $this->tbl_enquiry . '.enq_last_foll_cust_rmk Last_Customer_Remark',
                     $this->tbl_enquiry . '.enq_last_foll_date AS Last_Followup_On',
                     $this->tbl_enquiry . '.enq_home_visit_date AS Home_Visit_On',
                     $this->tbl_enquiry . '.enq_sm_rmk AS Last_SM_Remark',
                     $this->tbl_enquiry . '.enq_asm_rmk AS Last_ASM_Remark',
                     $this->tbl_enquiry . '.enq_mis_rmk AS Last_MIS_Remark',
                     "(CASE WHEN " . $this->tbl_enquiry . ".enq_cus_status = 1 THEN 'Sales'
                    WHEN " . $this->tbl_enquiry . ".enq_cus_status = 2 THEN 'Purchase'
                    WHEN " . $this->tbl_enquiry . ".enq_cus_status = 3 THEN 'Exchange' END) AS Enquiry_Type",
                     $this->tbl_users . '.usr_username AS Present_Sales_Staff',
                     $this->tbl_showroom . '.shr_location AS Showroom',
                     $this->tbl_occupation . '.occ_name AS Profession',
                     $this->tbl_city . '.cit_name AS Cust_Location',
                     'teamLead.usr_username AS Team_Lead',
                     $this->tbl_district_statewise . '.std_district_name AS District',
                     'createdby.usr_username AS Created_By',
                     $this->tbl_divisions . '.div_name AS Division',
                     $this->tbl_statuses . '.sts_title AS Enq_Status',
                     $this->tbl_enquiry_meta . '.enqm_pur_veh AS Purchase_Vehicle',
                     $this->tbl_enquiry_meta . '.enqm_sls_veh AS Sales_Vehicle',
                     $this->tbl_valuation . '.val_status AS Evaluated',
                     $this->tbl_contact_mode . '.cmd_title AS Mod_Of_Contact'
                 );
            } else {
                 $selectArray = array(
                     $this->tbl_enquiry . '.enq_id',
...
')->join('cpnl_enquiry', 'cpnl_enquiry.enq_id = cpnl_vehicle.veh_enq_id', 'LEFT')
       ...
       function salesExecutives() {
            ...
            $selectArray = array(
                ...
                $this->tbl_users . '.usr_last_name',
                $this->tbl_users . '.usr_username',
                $this->tbl_users . '.usr_id AS value',
                $this->tbl_users . '.usr_username AS label',
            );
       ...
       public function updateReAssignHistoryFollowup($fr, $to, $enq_id, $enquiry, $comment, $divBySE, $poolBatch) {
            ...
            if (!empty($fol)) {
                 ...
                 $this->db->where('enq_id', $enq_id)->update($this->tbl_enquiry, array(
                     ...
                     'enq_se_id' => $to,
                     'is_exe' => 1,
                     'enq_next_foll_date' => $nextFolDate,
                     'enq_current_status_history' => $hisId,
                     'enq_last_viewd' => 0,
                     'enq_is_pool' => 1,
                     'enq_pool_flag' => 1,
                     'enq_pool_entry_date' => date('Y-m-d H:i:s'),
                     'enq_pool_lst_cmd' => $comment,
                     'enq_pool_id' => $poolId
                 ));
...
WHERE   datediff(current_date,date(val_purchased_date)) $gl $no AND val_type=$type AND val_re_evaluated=0";
       ...
       public function statusWithHomeVisit($shrm = 1) { //rcd jsk shrm
            $assign = (int) assign_to_other_staff;
            $enqReOpnd = (int) inquiry_reopened;
            error_reporting(0);
       ...
       function weeklyTargetVsAchivement($shrm = 1, $status) { //rcd jsk shrm
            $sub = 1;
            // debug($status);
            /*
              SELECT cpnl_staff_targets.*,`cpnl_users`.`usr_showroom`,`cpnl_users`.`usr_first_name` FROM `cpnl_staff_targets` LEFT JOIN cpnl_users
       ...
       public function stockReport($shrm = 1) { //rcd jsk shrm
            error_reporting(0);
       ...
       public function expectBooking($shrm = 1) { //rcd jsk shrm
            error_reporting(0);
       ...
       public function expecDelivery($shrm = 1) { //rcd jsk shrm
            error_reporting(0);
       ...
       function getStffForHomeVisit() {

            if (check_permission('homevisit', 'hw_my_team')) {
                 $this->db->where($this->tbl_users . '.usr_tl', $this->uid);
            }
            if (check_permission('home_visit_report', 'hw_smart_only')) {
                 $this->db->where($this->tbl_users . '.usr_division', 1);
            }
       ...
       public function slEnqVsHmVisit() {

            $sql = "SELECT 
          sm.usr_username AS SM, 
          asm.usr_username AS ASM, 
          sales.usr_username AS Sales_Consultant, 
          sales.usr_id AS sales_Consultant_id 
      FROM 
          cpnl_users AS sm 
      LEFT JOIN 
          cpnl_users AS asm ON asm.usr_designation_new = 6 AND asm.usr_tl = sm.usr_id 
      LEFT JOIN 
          cpnl_users AS sales ON sales.usr_tl = asm.usr_id 
      WHERE 
          sm.usr_designation_new = 5
          ORDER BY 
     SM ASC, ASM ASC;
      ";
            //      $sql="SELECT 
            ...
            //      cpnl_users AS sm 
            //  LEFT JOIN 
            //      cpnl_users AS asm ON asm.usr_designation_new = 6 
            //  LEFT JOIN 
            //      cpnl_users AS sales ON sales.usr_id = sm.usr_tl 
            //  WHERE 
            //      sm.usr_designation_new = 5 
            //  ORDER BY 
            //      SM ASC, ASM ASC;
       ...
       function getMtdPaginate($postDatas, $sl_staff, $date = null) {
            ...
            if ($date) {
                 ...
                 // debug( $lastDayOfMonth);
            }

            $totalRecords = $this->getMtdTotal($searchValue, $sl_staff, $firstDayOfMonth, $lastDayOfMonth);

            // $this->db->where($this->tbl_source_funnel . '.sfnl_status', 1);


            $selArray = array(
                $this->re_assign_reports . '.re_id',
                $this->re_assign_reports . '.re_staff',
                $this->re_assign_reports . '.re_added_by',
                'assign.usr_first_name AS assign_usr_first_name',
                'assign.usr_last_name AS assign_usr_last_name',
                'addedby.usr_first_name AS addedby_usr_first_name',
                'addedby.usr_last_name AS addedby_usr_last_name',
            ...
            if (!empty($searchValue)) {
                 $this->db->where("(re_staff LIKE '%" . $searchValue . "%') ");
            }
            ...
            $data = $this->db->select($selArray)
                    ->where('re_staff', $sl_staff)
                    ->where('re_added_on >=', $firstDayOfMonth) // Filter by records added from the beginning of the current month
                    ->where('re_added_on <=', $lastDayOfMonth)  // Filter by records added till the end of the current month
                    ->join($this->tbl_users . ' assign', 'assign.usr_id = ' . $this->re_assign_reports . '.re_staff', 'LEFT')
                    ->join($this->tbl_users . ' addedby', 'addedby.usr_id = ' . $this->re_assign_reports . '.re_added_by', 'LEFT')
                    ->join($this->tbl_register_master, $this->tbl_register_master . '.vreg_id = ' . $this->re_assign_reports . '.re_reg_id', 'left')
                    ->join($this->tbl_brand, $this->tbl_brand . '.brd_id = ' . $this->tbl_register_master . '.vreg_brand', 'left')
                    ->join($this->tbl_model, $this->tbl_model . '.mod_id = ' . $this->tbl_register_master . '.vreg_model', 'left')
                    ->join($this->tbl_variant, $this->tbl_variant . '.var_id = ' . $this->tbl_register_master . '.vreg_varient', 'left')
                    ->get($this->re_assign_reports)
                    ->result_array();
            $response = array(
                "draw" => intval($draw),
                "iTotalRecords" => $totalRecords,
       ...
       function getMtdTotal($searchValue, $sl_staff, $firstDayOfMonth, $lastDayOfMonth) {

            $this->db->from($this->re_assign_reports)
                    ->where('re_staff', $sl_staff)
                    ->where('re_added_on >=', $firstDayOfMonth) // Filter by records added from the beginning of the current month
                    ->where('re_added_on <=', $lastDayOfMonth); // Filter by records added till the end of the current month
            if (check_permission('reports', 'reassignreport_showmyself')) {
            ...
            if (!empty($searchValue)) {
                 $this->db->like('re_staff', $searchValue);
            }
            return $this->db->count_all_results();
       }

       function getFtdPaginate($postDatas, $sl_staff, $date = null) {
            ...
            $totalRecords = $this->getFtdTotal($searchValue, $sl_staff, $today);
       ...
       function getFtdTotal($searchValue, $sl_staff, $date) {


            $this->db->from($this->re_assign_reports)->where('DATE(re_added_on)', $date)->where('re_staff', $sl_staff);
            if (check_permission('reports', 'reassignreport_showmyself')) {
                 $this->db->where('cpnl_re_assign_reports.re_added_by', $this->uid);
            }
            if (!empty($searchValue)) {
       ...
       function getAsmData($asm, $date = null) { //asm Or apm
            $today_date = date('Y-m-d');
            $first_day_of_month = date('Y-m-01');
            if ($date) {
                 $today_date = date('Y-m-d', strtotime($date));
            ...
            $this->db->select('cpnl_re_assign_reports.re_added_by,
          SUM(DATE(cpnl_re_assign_reports.re_added_on) = "' . $today_date . '") as TodayCountAsm,
          SUM(DATE(cpnl_re_assign_reports.re_added_on) >= "' . $first_day_of_month . '") as ThisMonthCountAsm');
            $this->db->from('cpnl_re_assign_reports');
            $this->db->where('cpnl_re_assign_reports.re_staff', $asm);
            if (check_permission('reports', 'reassignreport_showmyself')) {
                 $this->db->where('cpnl_re_assign_reports.re_added_by', $this->uid);
            }

            $queryAsm = $this->db->get();
            $rowAsm = $queryAsm->row();
       ...
       function getReAssignPurchaseData($date = null) {
            $today_date = date('Y-m-d');
            // Get the first day of the current month
            $first_day_of_month = date('Y-m-01');
            if ($date) {
            ...
            foreach ($showrooms as $showroom) {
                 ...
                 $enquiry_count = 0;
                 $salesManagers = $this->db //purchase Manager //Pm
                         ->select('usr_id,usr_username')
                         ->where_in('usr_designation_new', [22, 81])
                         ->where('usr_showroom', $showroom->shr_id)
                         ->where('usr_active', 1)->where('usr_resigned', 0) //jsk
                         ->get('cpnl_users')
                         ->result(); //ixd
                 ...
                 foreach ($salesManagers as $salesManager) {
                      ...
                      $asms = $this->db //Apm
                              ->select('usr_id,usr_username')
                              // ->where('(usr_designation_new = 6)') //  OR usr_designation_new = 5
                              ->where_in('usr_designation_new', [24]) //jsk
                              ->where('usr_tl', $salesManager->usr_id)
                              ->where('usr_active', 1)->where('usr_resigned', 0) //
                              ->get('cpnl_users')
                              ->result(); //idx 
                      ...
                      foreach ($asms as $asm) {
                           ...
                           $salesConsultants = $this->db
                                   ->select('usr_id,usr_username')
                                   ->where_in('usr_designation_new', [35, 40, 69]) //Purchase Exicutives
                                   ->where('usr_tl', $asm->usr_id)
                                   ->where('usr_active', 1)->where('usr_resigned', 0) //
                                   ->get('cpnl_users')
                                   ->result(); //idx
                 ...
                 $infos[] = [
                     ...
                     'today_count' => $today_count,
                     'this_month_count' => $this_month_count,
                     'today_count_asm' => $today_count_asm,
                     'this_month_count_asm' => $this_month_count_asm,
                         //  'today_count_asm'=>  $asm->today_count_asm,
                         // 'this_month_count_asm' => $asm->this_month_count_asm
                 ];
                 ...
                 // $total_today_count_asm += $today_count_asm;
                 // $total_this_count_asm +=  $this_month_count_asm;
            }

            $total_count_info = [
                'today_count' => $total_today_count,
                'this_month_count' => $total_this_count,
                    // 'today_count_asm' => $total_today_count_asm,
                    // 'this_month_count_asm' => $total_this_count_asm,
            ];
            return [
                'infos' => $infos,
                'total_count_info' => $total_count_info,
       ...
       function allowenquiryforcedrop($data) {
            $salesStaffName = $data['usr_first_name'];
            $custName = $data['enq_cus_name'];
            unset($data['usr_first_name']);
            unset($data['enq_cus_name']);
       ...
       function asmByDivision() {
            //6 - ASM
            //79 - Territory Sales Manager
            //82 - Deputy Sales Manager
            //83 - Regional Sales Manager
            //99 - Assistant Sales Manager
            //100 - Area Sales Manager

            return $this->db->select('usr_id,usr_username,usr_designation_new')->where('usr_division', $this->div)
                            ->where(array('usr_active' => 1, 'usr_resigned' => 0))
                            ->where_in('usr_designation_new', array(6, 79, 82, 99, 100))
                            ->get($this->tbl_users)->result_array();
       }

       public function getShowrooms($id = '', $div = array()) {
            if (!empty($id)) {
                 $this->db->where('shr_id', $id);
                 return $this->db->get($this->tbl_showroom)->row_array();
            }
            if (!empty($div)) {
                 $this->db->where('(shr_division = ' . implode(' OR shr_division = ', $div) . ')');
            }
            return $this->db->select('CONCAT(' . $this->tbl_showroom . '.shr_location, ' . "' ('" . ',' . $this->tbl_divisions . '.div_name,' . "')'" . ') AS label, ' .
                                    $this->tbl_showroom . '.shr_id AS value', false)
                            ->join($this->tbl_divisions, $this->tbl_divisions . '.div_id = ' . $this->tbl_showroom . '.shr_division', 'LEFT')
                            ->where('shr_status', 1)->get($this->tbl_showroom)->result_array();
       }

       function gatepassReportAjax($postDatas) {
            $draw = $postDatas['draw'];
            $row = $postDatas['start'];
            $rowperpage = $postDatas['length']; // Rows display per page
            $columnIndex = isset($postDatas['order'][0]['column']) ? $postDatas['order'][0]['column'] : ''; // Column index
            ...
            $fields = array(
                ...
                $this->tbl_vehicle_booking_master . '.vbk_cust_name',
                $this->tbl_vehicle_booking_master . '.vbk_ref_no',
                'booking_added_by.usr_username AS booking_added_username'
            );
            $data = $this->db->select($fields)
                            ...
                            ->join($this->tbl_users . ' booking_added_by', 'booking_added_by.usr_id = ' . $this->tbl_vehicle_booking_master . '.vbk_added_by', 'left')
                            ->where($this->tbl_tracking . '.trk_booking_no > 0')->order_by('trk_id', 'DESC')->get($this->tbl_tracking)->result_array();
            // Count total records
