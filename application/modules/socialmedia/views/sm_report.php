                         ...
                         <form action="<?php echo site_url('reports/re_assign_report/');?>" method="get">
                              <table style='border:none;'>
                                   <td style='border:none;'>
                                        <input autocomplete="off" name="date" type="text" value="<?php echo $today; ?>" class="dtpEnquiry form-control col-md-7 col-xs-12" 
                                               placeholder="Date"/>
                                   </td>
...
function loadAjaxContent() {
            ...
            $.ajax({
                type: 'get',
                url: site_url + "socialmedia/sm_report_ajx",
                data: { date: dateValue }, 
...
<script>
     ...
     function saveToDatabase(editableObj, master_id,date) {
       // alert(date);
          $(editableObj).css("background", "#FFF url(https://www.rdms.royaldrive.in/assets/images/loaderIcon.gif) no-repeat center right 5px");
          $.ajax({
               url: site_url + "socialmedia/store_followers",
               type: "POST",
               data: 'master_id=' + master_id + '&followers=' + parseInt(editableObj.innerHTML)+ '&date=' + date,
    ...
    // Allow only numeric input (0-9) and control keys (backspace, delete, arrow keys)
