...
class followupdemo_model extends CI_Model
{
     ...
     function getFollowupByEnquiry($enqId)
     {
          if (!empty($enqId)) {
               ...
               if (!empty($enq)) {
                    ...
                    $enq['followups'] = $this->db->select($this->tbl_followup . '.foll_id,foll_next_foll_date,foll_entry_date,
                         foll_customer_feedback,foll_remarks,foll_contact,foll_action_plan,foll_budget_from,foll_budget_to,foll_km_from,
                         foll_km_to,foll_model_y_from,foll_model_y_to,foll_is_cmnt,foll_parent,foll_status,' .
                         ...
                         ->order_by($this->tbl_followup . '.foll_id', 'DESC')->get_where(
                              $this->tbl_followup,
                              array($this->tbl_followup . '.foll_cus_id' => $enqId, 'foll_parent' => 0)
                         )->result_array();
     ...
     function getStockVehicles()
     {
          $selectData = array(
               ...
               $this->tbl_valuation . '.val_comp_stock',
     ...
     function getComments($follId)
     {
          ...
          return $this->db->select($this->tbl_followup . '.foll_entry_date, ' . $this->tbl_followup . '.foll_remarks, ' .
               $this->tbl_users . '.usr_username AS folloup_added_by, ' .
     ...
     public function getStates($id = '')
     {
          if ($id) {
               ...
               return $this->db->select($this->tbl_states . '.*')->get($this->tbl_states)->result_array();
          }
     }
