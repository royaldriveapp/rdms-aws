...
class followuptest extends App_Controller
{
     ...
     public function __construct()
     {
          ...
          $this->load->model('followuptest_model', 'followup');
     ...
     function viewFollowupDhl($enqId)
     {
          //$this->db->cache_off();
...
function get_followups($enqId){
     if (!empty($enqId)) {
         $data['vehicles'] = $this->followup->getFollowupByEnquiry($enqId);
        //debug($data['vehicles']);
        $result = $this->load->view('ajx/followup_ajx', $data);
    // $result = $this->load->view('ajx/followup_test', $data);
...
function viewFollowup($enqId)
     {
          ...
          if (!empty($enqId)) {
             ...
             //  print_r($data['enqInfo']); exit;
              $this->render_page(strtolower(__CLASS__) . '/followup_new1', $data);
          } else {
               //404
          }
     }
function get_followups_new($enqId){
     ...
     if (!empty($enqId)) {
       ...
       //  $data['enq_id']=$enqId;
        ...
        //debug($data['vehicles']);
        $result = $this->load->view('ajx/ajx_new1', $data);
    // $result = $this->load->view('ajx/followup_test', $data);
          return json_encode($result);
     } else {
          //404
...
function viewFollowup22($enqId)
{
     ...
     if (!empty($enqId)) {
          $data['enqInfo'] = $this->followup->getEnqInfo($enqId);
         $this->render_page(strtolower(__CLASS__) . '/followup2', $data);
     } else {
          //404
     }
}
function viewFollowup2($enqId)
{ //debug();
     //$this->db->cache_off();
     $data['statuses'] = $this->common_model->getStatuses('vehicle');
     $data['brands'] = array();
     ...
     //test//
     function viewFollowup_test($enqId)
     {
          ...
          if (!empty($enqId)) {
               ...
               $data['staffs'] = $this->followup->getStaffs();
               if (is_roo_user()) {
                    $this->render_page(strtolower(__CLASS__) . '/followup_test', $data);
               } else {
                    $this->render_page(strtolower(__CLASS__) . '/followup_test', $data);
               }
          } else {
