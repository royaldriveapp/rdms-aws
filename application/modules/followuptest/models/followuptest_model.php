...
class followuptest_model extends CI_Model
{
     ...
     {
          return $this->db->order_by('pr_sort_order')->get($this->tbl_price_range)->result_array();
     }
     ...
     {
          return $this->db->select('enq_se_id,enq_cus_test_drive')->where($this->tbl_enquiry . '.enq_id', $id)->get($this->tbl_enquiry)->row_array();
     }

     function getFollowupEnquires($id = '', $limit = 0, $page = 0, $filters = '')
     {
          ...
          if (!empty($id)) {
               ...
               if (!empty($enq)) {
                    ...
                    //      ->get($this->view_vehicle_vehicle_status)->result_array();//old
                    ...
                    // old  $enq['vehicle_buy'] = $this->db->select($this->view_vehicle_vehicle_status . '.*')
               ...
               if (isset($filters['enq_id']) && !empty($filters['enq_id'])) {
                    $this->db->where($this->tbl_enquiry . '.enq_se_id = ' . $this->uid);
               } else {
                    if ($this->usr_grp == 'AD') {
                    } else if ($this->usr_grp == 'TL' || check_permission('enquiry', 'showonlymystaff')) {
                    ...
                    if (isset($filters['foll_next_foll_date']) && !empty($filters['foll_next_foll_date'])) {
                         $date = date('Y-m-d', strtotime($filters['foll_next_foll_date']));
                         $this->db->where('DATE(' . $this->tbl_enquiry . '.enq_next_foll_date) =', $date);
                    }
                    if (isset($filters['enq_is_pool']) && !empty($filters['enq_is_pool'])) {
                         $this->db->where($this->tbl_enquiry . '.enq_is_pool', 1);
                    }

                    if (isset($filters['enq_se_id']) && !empty($filters['enq_se_id'])) {
                         $this->db->where($this->tbl_enquiry . '.enq_se_id', $filters['enq_se_id']);
                    }
               }
               $this->db->where_in($this->tbl_enquiry . '.enq_current_status', $this->myEnqStatuses);
               if (isset($filters['search']) && !empty($filters['search'])) {
                    $whereSearch = '(' . $this->tbl_enquiry . ".enq_cus_name LIKE '%" . $filters['search'] . "%' OR " .
                         $this->tbl_enquiry . ".enq_number LIKE '%" . $filters['search'] . "%' OR " .
                         $this->tbl_enquiry . ".enq_cus_mobile LIKE '%" . $filters['search'] . "%' OR " .
                         $this->tbl_enquiry . ".enq_cus_whatsapp LIKE '%" . $filters['search'] . "%' )";
               ...
               if (isset($filters['search']) && !empty($filters['search'])) {
                    $whereSearch = '(' . $this->tbl_enquiry . ".enq_cus_name LIKE '%" . $filters['search'] . "%' OR " .
                         $this->tbl_enquiry . ".enq_number LIKE '%" . $filters['search'] . "%' OR " .
                         $this->tbl_enquiry . ".enq_cus_mobile LIKE '%" . $filters['search'] . "%' OR " .
                         $this->tbl_enquiry . ".enq_cus_whatsapp LIKE '%" . $filters['search'] . "%' OR " .
                         $this->tbl_enquiry_meta . ".enqm_pur_veh LIKE '%" . $filters['search'] . "%' OR " .
                         $this->tbl_enquiry_meta . ".enqm_sls_veh LIKE '%" . $filters['search'] . "%' )";
                    $this->db->where($whereSearch);
               }
               $selectArray = array(
                    ...
                    $this->tbl_enquiry_meta . '.enqm_pur_veh', $this->tbl_enquiry_meta . '.enqm_sls_veh'
               );
               if ($limit) {
     ...
     /*  function getFollowupByEnquiryBkp($enqId)
     {
          if (!empty($enqId)) {
               $select = array(
                    $this->tbl_enquiry . '.enq_cus_name',
                    ...
                    $this->tbl_enquiry . '.enq_cus_mobile',
                    $this->tbl_enquiry . '.enq_cus_status', //added by jsk
                    // $this->tbl_occupation . '.*',
                    // $this->tbl_city . '.*',
                    // $this->tbl_district . '.*',
                    // $this->tbl_state . '.*',
                    ...
                    $this->tbl_users . '.usr_first_name',
                    $this->tbl_statuses . '.sts_value',
                    $this->tbl_statuses . '.sts_title',
                    $this->tbl_statuses . '.sts_des',
                    $this->tbl_statuses . '.sts_slug',
               );
               ...
               //debug($enq);
               if (!empty($enq)) {
                    $enq['vehicle_sale'] = $this->db->select($this->tbl_vehicle . '.veh_id, ' . $this->tbl_brand . '.brd_title,' .
                    ...
                    $enq['vehicle_buy'] = $this->db->select($this->tbl_vehicle . '.veh_id, ' . $this->tbl_brand . '.brd_title,' .
                         $this->tbl_model . '.mod_title,' . $this->tbl_variant . '.var_variant_name')
                         ->join($this->tbl_brand, $this->tbl_brand . '.brd_id = ' . $this->tbl_vehicle . '.veh_brand', 'LEFT')
                         ->join($this->tbl_model, $this->tbl_model . '.mod_id = ' . $this->tbl_vehicle . '.veh_model', 'LEFT')
                         ->join($this->tbl_variant, $this->tbl_variant . '.var_id = ' . $this->tbl_vehicle . '.veh_varient', 'LEFT')
     ...
     function getFollowupByEnquiryAjXNw($enqId)
     {
         // debug($enqId);
          if (!empty($enqId)) {
               // $select = array(
              ...
              //$enq=array();
               //if (!empty($enq)) {
                    ...
                    $enq['followups'] = $this->db->select($this->tbl_followup . '.foll_id,foll_next_foll_date,foll_entry_date,foll_customer_feedback,foll_remarks,foll_contact,foll_action_plan,foll_budget_from,foll_budget_to,foll_km_from,foll_km_to,foll_model_y_from,foll_model_y_to,foll_is_cmnt,foll_parent,foll_status,' .
                         $this->tbl_vehicle . '.veh_brand, ' . $this->tbl_vehicle . '.veh_model, ' .
                         $this->tbl_brand . '.brd_title,' . $this->tbl_vehicle . '.veh_varient,' .
                         $this->tbl_model . '.mod_title,' . $this->tbl_variant . '.var_variant_name,' .
                         $this->tbl_users . '.usr_username AS folloup_added_by, ' . $this->tbl_users . '.usr_id AS folloup_added_by_id,' .
                    ...
                    $enq['vehicle_sale'] = $this->db->select($this->tbl_vehicle . '.veh_id, ' . $this->tbl_brand . '.brd_title,' .
                         ...
                         ->join($this->tbl_model, $this->tbl_model . '.mod_id = ' . $this->tbl_vehicle . '.veh_model', 'LEFT')
                         ->join($this->tbl_variant, $this->tbl_variant . '.var_id = ' . $this->tbl_vehicle . '.veh_varient', 'LEFT')
                         ->get_where($this->tbl_vehicle, array($this->tbl_vehicle . '.veh_enq_id' => $enqId, $this->tbl_vehicle . '.veh_status' => 1))->result_array();
                    ...
                    $enq['vehicle_buy'] = $this->db->select($this->tbl_vehicle . '.veh_id, ' . $this->tbl_brand . '.brd_title,' .
                         ...
                         ->join($this->tbl_model, $this->tbl_model . '.mod_id = ' . $this->tbl_vehicle . '.veh_model', 'LEFT')
                         ->join($this->tbl_variant, $this->tbl_variant . '.var_id = ' . $this->tbl_vehicle . '.veh_varient', 'LEFT')
                         ->get_where($this->tbl_vehicle, array($this->tbl_vehicle . '.veh_enq_id' => $enqId, $this->tbl_vehicle . '.veh_status' => 2))->result_array();
                    //Question answers
                    $enq['questionAnswers'] = array();
                    if ($enq['enq_cus_status'] == 1) { // Sale
                         $enq['questionAnswers'] = $this->db->select($this->tbl_questions . '.qus_id,qus_is_mandatory,qus_is_togler,qus_desc,' . $this->tbl_enquiry_questions . '.enqq_id,enqq_answer')
                              ->join($this->tbl_enquiry_questions, $this->tbl_enquiry_questions . '.enqq_question_id = ' . $this->tbl_questions . '.qus_id', 'LEFT ')
                              ->order_by($this->tbl_questions . '.qus_order')
                              ->get_where($this->tbl_questions, '(' . $this->tbl_questions . '.qus_type = 1 OR ' . $this->tbl_questions . '.qus_type = 2) AND (' .
                                   $this->tbl_enquiry_questions . '.enqq_enq_id = ' . $enqId . ' OR cpnl_enquiry_questions.enqq_enq_id IS NULL)')
     ...
     function addFollowUp($datas, $lstFollCmd = '')
     {
          if (!empty($datas)) {
               ...
               if ($this->db->insert($this->tbl_followup, $datas)) {
                    ...
                    if ((isset($datas['foll_cus_id']) && !empty($datas['foll_cus_id'])) &&
                         ...
                         $this->db->update(
                              ...
                              array(
                                   ...
                                   'enq_last_foll_date' => date('Y-m-d H:i:s'),
                                   'enq_last_foll_cust_rmk' => $lstFollCmd,
                                   'enq_last_followup_by' => $this->uid
                              )
                         );
                    ...
                    /* Update budget on enquiry */
                    if (isset($datas['foll_budget']) && !empty($datas['foll_budget'])) {
                         $this->db->where('enq_id', $datas['foll_cus_id']);
                         $this->db->update($this->tbl_enquiry, array('enq_budget_id' => $datas['foll_budget']));
                    }
                    return true;
               }
          } else {
     ...
     function missedCount()
     {
          ...
          if ($designation) {
               $this->db->where_in($this->tbl_users . ".usr_designation_new", $designation);
          }
          if (check_permission('upcommingfollowup', 'exclude_cold')) {
               $this->db->where('(enq_cus_when_buy = 1 OR enq_cus_when_buy = 2 OR enq_cus_when_buy = 3)');
          }
          if (check_permission('followup', 'showresignedstaff')) {
               $this->db->where('( ' . $this->tbl_users . '.usr_active = 0 OR ' . $this->tbl_users . '.usr_active = 1) AND (' .
                    $this->tbl_users . '.usr_resigned = 0 OR ' . $this->tbl_users . '.usr_resigned = 1)');
          } else {
          ...
          if ($this->desi == 'SH' || check_permission('enquiry', 'showonlymystaff')) {
               $return = $this->db->where_in($this->tbl_enquiry . '.enq_se_id', $mystaff)
                    ->group_by($this->tbl_enquiry . '.enq_se_id')->get($this->tbl_enquiry)->result_array();
          } else if (check_permission('followup', 'showmisdfolcount_myshowrom')) {
     ...
     function missedCountYTD()
     {
          ...
          $this->db->select($selectArray)
               ...
               //->where(array($this->tbl_groups . '.grp_slug' => 'SE'))
               ->where_in($this->tbl_enquiry . '.enq_current_status', $this->myEnqStatuses)
               ->where('(DATEDIFF(DATE(' . $this->tbl_enquiry . '.enq_next_foll_date), ' . "DATE('" . date('Y-m-d', strtotime("-1 days")) . "')" . ') <= -3)')
               ->where(array($this->tbl_users . '.usr_active' => 1, $this->tbl_users . '.usr_resigned' => 0));
          if ($this->desi == 'SH') {
               $return = $this->db->where_in($this->tbl_enquiry . '.enq_se_id', $mystaff)
                    ->group_by($this->tbl_enquiry . '.enq_se_id')->get($this->tbl_enquiry)->result_array();
     ...
     function missed($search = '', $limit = 0, $page = 0, $salesStaff)
     {
          $showroom = get_logged_user('usr_showroom');
          if ($this->usr_grp == 'TL' || check_permission('followup', 'misdfolupshowmyteam')) {
               $mystaffs = my_staff($this->uid);
          ...
          if ($this->usr_grp == 'SE') {
               ...
               $this->db->where($this->tbl_enquiry . '.enq_showroom_id = ' . $showroom);
          } else if ($this->usr_grp == 'TL' || check_permission('followup', 'misdfolupshowmyteam')) {
          ...
          if (!empty($salesStaff)) {
               $this->db->where($this->tbl_enquiry . '.enq_se_id', $salesStaff);
          }
          $return['count'] = $this->db->select($this->tbl_enquiry . '.enq_id,' .
     ...
     function editFollowUp($datas)
     {
          ...
          $budget = (isset($datas['followup']['foll_budget']) && !empty($datas['followup']['foll_budget'])) ? $datas['followup']['foll_budget'] : 0;
          $custStatus = (isset($datas['followup']['foll_status']) && !empty($datas['followup']['foll_status'])) ? $datas['followup']['foll_status'] : 0;
          generate_log(array(
               'log_title' => 'New followup',
               'log_desc' => serialize($datas),
          ...
          $lstFollCmd = isset($datas['foll_customer_feedback']) ? implode(',', $datas['foll_customer_feedback']) : '';
          if ((isset($datas['assignto']) && !empty($datas['assignto'])) && (isset($datas['foll_cus_id']) && !empty($datas['foll_cus_id']))) {
               ...
               if (isset($datas['p']) && !empty($datas['p'])) {
                    $enqUpdPool['enq_pool_updt_date'] = date('Y-m-d H:i:s');
                    $enqUpdPool['enq_pool_lst_cmd'] = implode(', ', $datas['foll_customer_feedback']);
                    $enqUpdPool['enq_pool_flag'] = 0; //Not show in enq pool
                    $enqUpdPool['enq_cus_when_buy'] = $custStatus;
                    $this->db->where('enq_id', $datas['foll_cus_id'])->update($this->tbl_enquiry, $enqUpdPool);
                    $this->db->where('enp_id', $datas['p'])->update($this->tbl_enquiry_pool, array(
                         'enp_updated_on' => date('Y-m-d H:i:s'),
          ...
          if (isset($datas['followup']['foll_next_foll_date']) && !empty($datas['followup']['foll_next_foll_date'])) {
               ...
               if (isset($datas['foll_customer_feedback']) && !empty($datas['foll_customer_feedback'])) {
                    $lstFollCmd = implode(',', $datas['foll_customer_feedback']);
                    foreach ($datas['foll_customer_feedback'] as $key => $value) {
                         ...
                         if (empty($follupAlreadyEntered)) { // Create new followup
                              ...
                              if ((isset($datas['followup']['foll_status']) && !empty(isset($datas['followup']['foll_status']))) &&
                                   ...
                                   $this->addFollowUp($datas['followup'], $lstFollCmd);
                              }
                    ...
                    /*Update last followup update details*/
                    $datas['foll_next_foll_date'] = (isset($datas['followup']['foll_next_foll_date']) && !empty($datas['followup']['foll_next_foll_date'])) ?
                         date('Y-m-d h:i:s', strtotime($datas['followup']['foll_next_foll_date'])) : '';

                    $follStatus = isset($datas['followup']['foll_status']) ? $datas['followup']['foll_status'] : 0;
                    $this->db->where('enq_id', $datas['foll_cus_id']);
               ...
               if (!empty($budget)) {
                    $this->db->where('enq_id', $datas['foll_cus_id']);
                    $this->db->update($this->tbl_enquiry, array('enq_budget_id' => $budget));
               }
               return true;
          }
          return false;
     ...
     function changeStatus($data, $get = '')
     {
          if (!empty($data)) {
               ...
               if ($this->db->insert($this->tbl_enquiry_history, array_filter($data))) {
               ...
               foreach ($misdFolEnqId as $key => $enqId) {
                    ...
                    if (!empty($enqDetails)) {
                         ...
                         foreach ((array) $vehicles as $key => $veh) {
                              ...
                              $this->db->update($this->tbl_enquiry, array('enq_next_foll_date' => $nextFollowupDetails['foll_next_foll_date'], 'enq_last_foll_date' => date('Y-m-d H:i:s')));
     ...
     function getStockVehicles()
     {
          $selectData = array(
               ...
               $this->tbl_valuation . '.val_price_rd_to',
               $this->tbl_model . '.mod_title',
               $this->tbl_brand . '.brd_title',
               $this->tbl_variant . '.var_variant_name'
          );
          $this->db->where('((' .  $this->tbl_valuation . '.val_status = ' . add_stock . ' OR ' . $this->tbl_valuation . '.val_status = ' . vehicle_evaluated . ' OR ' .
               $this->tbl_valuation . '.val_status = 28 OR ' . $this->tbl_valuation . '.val_status = 13) AND ' . $this->tbl_valuation . '.val_is_booked = 0)');
          //$this->tbl_valuation . '.val_status = 1 OR ' .
     ...
     {
          $select = array(
               $this->tbl_enquiry_history . '.enh_added_on',
               $this->tbl_enquiry . '.enq_id',
               $this->tbl_users . '.usr_id',
               $this->tbl_statuses . '.sts_des',
     ...
     {
          return $this->db->select('vreg_division')->order_by('vreg_id', 'DESC')->limit(1)->get_where($this->tbl_register_master, array('vreg_inquiry' => $enqId))->row_array();
     }
     ...
     function updateComments($data)
     {
          ...
          //update last comment on enquiry
          $rmkField = '';
          $rmkFieldTime = '';
          if ($this->desi == 'SM' || $this->desi == 'PM' || $this->desi == 'DSM') {
               $rmkField = 'enq_sm_rmk';
               $rmkFieldTime = 'enq_sm_rmk_time';
          } else if ($this->desi == 'ASM' || $this->desi == 'APM') {
               $rmkField = 'enq_asm_rmk';
               $rmkFieldTime = 'enq_asm_rmk_time';
          } else if ($this->desi == 'MIS' || $this->desi == 'TC') {
               $rmkField = 'enq_mis_rmk';
               $rmkFieldTime = 'enq_mis_rmk_time';
          }

          if (!empty($rmkField)) {
               $this->db->where('enq_id', $data['foll_cus_id'])->update($this->tbl_enquiry, array($rmkField => $data['foll_remarks'], $rmkFieldTime => date('Y-m-d H:i:s')));
          }

          return true;
     }

     function getCommentsBk($follId)
     {
     ...
     function getCommentskk($follId)
     {

    $this->db->select([
               'f.folloup_added_by_id',
               'f.folloup_added_by',
               'f.foll_remarks',
               'f.foll_entry_date',
               'u.usr_username AS folloup_added_by',
               'u.usr_id AS folloup_added_by_id',
               'u.usr_avatar',
               's.shr_location'
           ])
           ->from("$this->tbl_followup f")
           ->join("$this->tbl_vehicle v", 'v.veh_id = f.foll_cus_vehicle_id', 'LEFT')
           ->join("$this->tbl_brand b", 'b.brd_id = v.veh_brand', 'LEFT')
           ->join("$this->tbl_model m", 'm.mod_id = v.veh_model', 'LEFT')
           ->join("$this->tbl_variant var", 'var.var_id = v.veh_varient', 'LEFT')
           ->join("$this->tbl_users u", 'u.usr_id = f.foll_added_by', 'LEFT')
           ->join("$this->tbl_showroom s", 's.shr_id = u.usr_showroom', 'LEFT')
           ->where('f.foll_parent', $follId)
           ->order_by('f.foll_id', 'DESC')
           ->get()
           ->result_array();
     ...
     function getPreferences($id = '')
     {
          ...
          if ($id) {
               return $this->db->select($this->tbl_enq_prefrences . '.prf_key,' . $this->tbl_enq_prefrences . '.prf_description,' .
                    $this->tbl_enq_prefrences . '.prf_value,' .
     ...
     function getPrequrementRqst($id = '', $filter)
     {
          if ($id) {
          ...
          if (isset($filter['enq_date_from']) && !empty($filter['enq_date_from'])) {
               ...
               $this->db->where('DATE(' . $this->tbl_procurement_rqsts . '.proc_added_on) >=', $date);
               $this->db->order_by($this->tbl_procurement_rqsts . '.proc_added_on');
          }
          ...
          if (isset($filter['executive']) && !empty($filter['executive'])) {
               $this->db->where_in($this->tbl_procurement_rqsts . '.proc_addded_by', $filter['executive']);
          }
     ...
     function getStaffs()
     {
          ...
          $salesStaff = $this->db->select($this->tbl_users . '.usr_id AS col_id, ' . $this->tbl_users . '.usr_first_name AS col_title, ' .
               ...
               ->join($this->tbl_showroom, $this->tbl_showroom . '.shr_id = ' . $this->tbl_users . '.usr_showroom', 'left')
               ->where('(' . $this->tbl_groups . ".grp_slug = 'SE' OR " . $this->tbl_groups . ".grp_slug = 'DE' OR " . $this->tbl_groups . ".grp_slug = 'TL' OR " . $this->tbl_groups . ".grp_slug = 'MG')")
               ->where(array($this->tbl_users . '.usr_active' => 1, $this->tbl_users . '.usr_resigned' => 0))
               ->where($this->tbl_users . '.usr_active', 1)->where($this->tbl_users . '.usr_resigned', 0)
     ...
     function addHomeVisit($data)
     {
          ...
          if (!empty($data)) {
               ...
               if ($this->db->insert($this->tbl_home_visits, $data)) {
                    ...
                    if (isset($data['hmv_enq_id']) && !empty($data['hmv_date'])) {
                         $this->db->where('enq_id', $data['hmv_enq_id'])->update('cpnl_enquiry', array('enq_cus_home_visit' => 1, 'enq_home_visit_date' => $data['hmv_date']));
                    }
                    return true;
               }
          }
     ...
     function getHomeVisitList($filter)
     {
          //
          if (isset($filter['hmv_date_from']) && !empty($filter['hmv_date_from'])) {
               $date = date('Y-m-d', strtotime($filter['hmv_date_from']));
               $this->db->where('DATE(' . $this->tbl_home_visits . '.hmv_date) >=', $date);
               $this->db->order_by($this->tbl_home_visits . '.hmv_date');
          }
          if (isset($filter['hmv_date_to']) && !empty($filter['hmv_date_to'])) {
               $date = date('Y-m-d', strtotime($filter['hmv_date_to']));
               $this->db->where('DATE(' . $this->tbl_home_visits . '.hmv_date) <=', $date);
          }
     ...
     /*function getHomeVisitApprovelReq($hmv_id = '')
     {
          ...
          $qry = $this->db->select($this->tbl_home_visits . '.*,' .
               ...
               ->where("(" . $this->tbl_home_visits_approvals . '.hmva_approved_by IS NULL' . " or cpnl_home_visit_approvals.hmva_approval_status=2)");
     ...
     function getHomeVisitApprovelReq($hmv_id = '')
     {
          ...
          $qry = $this->db->select($this->tbl_home_visits . '.*,' .
               ...
               ->join($this->tbl_home_visits_approvals, $this->tbl_home_visits_approvals . '.hmva_master_id = ' .
                    $this->tbl_home_visits . '.hmv_id AND ' .
                    $this->tbl_home_visits_approvals . '.hmva_approved_by = ' . $this->uid, 'LEFT') //"your_id !=",$your_id
               ->where("(" . $this->tbl_home_visits_approvals . '.hmva_approved_by IS NULL' . " or $this->tbl_home_visits_approvals.hmva_approval_status=2)");

          if ($this->usr_grp == 'TL') {
               $this->db->where_in($this->tbl_home_visits . '.hmv_added_by', $mystaffs);
          }
          //$qaryfltr
          // $this->db->where($this->tbl_home_visits . '.hmv_added_by', $this->uid);
     ...
     function updateHomeVisitApproval($data)
     {
          ...
          /*Update Home visit on master*/
          $this->homeVisitApprovalToMaster($data['hmva_master_id']);
          return true;
     }
     function homeVisitApprovalToMaster($hwId)
     {
          $appJson = json_encode($this->db->select($this->tbl_home_visits_approvals . '.*,' . $this->tbl_users . '.usr_username')
               ->join($this->tbl_users, 'usr_id = hmva_approved_by', 'left')->order_by('hmva_id', 'DESC')
               ->where('hmva_master_id', $hwId)->get($this->tbl_home_visits_approvals)->result_array());

          $this->db->where('hmv_id', $hwId)->update($this->tbl_home_visits, array('hmv_approvals' => $hwa));
     }
