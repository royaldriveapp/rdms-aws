<?php
  ...
  class Ion_auth_model extends CI_Model {
       ...
       public function login($identity, $password, $remember = FALSE) {
            ...
            $query = $this->db->select($this->identity_column . ', usr_username, usr_email, usr_id, usr_password, usr_active, usr_last_login,' . $this->tables['user_access'] . '.cua_access AS user_access')
                    ->join($this->tables['user_access'], $this->tables['user_access'] . '.cua_user_id = ' . $this->tables['users'] . '.usr_id', 'left')
       ...
       public function users($groups = NULL) {
            ...
            if (isset($this->_ion_select) && !empty($this->_ion_select)) {
                 ...
                 $this->db->select(array(
                     $this->tables['users'] . '.*',
                     $this->tables['users'] . '.usr_id as id',
                     $this->tables['users'] . '.usr_id as user_id',
                     TABLE_PREFIX . 'city.*',
            ...
            $tblUser = $this->tables['users'];

            $this->db->join(
                    TABLE_PREFIX . 'city', TABLE_PREFIX . 'city.cit_id = ' . $tblUser . '.usr_city', 'left'
            );
            $this->db->join(
                    TABLE_PREFIX . 'country', TABLE_PREFIX . 'country.cnt_id = ' . $tblUser . '.usr_country', 'left'
            );
            $this->db->join(
                    TABLE_PREFIX . 'state', TABLE_PREFIX . 'state.stt_id = ' . $tblUser . '.usr_state', 'left'
            );
            $this->db->join(
                    TABLE_PREFIX . 'district', TABLE_PREFIX . 'district.dit_id = ' . $tblUser . '.usr_district', 'left'
            );
            ...
            if (isset($groups)) {
                 ...
                 if (isset($groups) && !empty($groups)) {
                      $this->db->distinct();
                      $this->db->join(
                              $this->tables['users_groups'], $this->tables['users_groups'] . '.' . $this->join['users'] . '=' . $this->tables['users'] . '.usr_id', 'inner'
                      );

                      $this->db->join(
                              $this->tables['user_access'], $this->tables['user_access'] . '.cua_user_id' . '=' . $this->tables['users'] . '.usr_id', 'left'
                      );
       ...
       public function update($id, array $data) {
            ...
            if (array_key_exists('usr_password', $data) || array_key_exists('usr_email', $data)) {
                 if (array_key_exists('usr_password', $data)) {
                      if (!empty($data['usr_password'])) {
                           $data['usr_password'] = $this->hash_password($data['usr_password'], $user->usr_salt);
                      } else {
            ...
            $this->trigger_events('extra_where');
            $this->db->update($this->tables['users'], $data, array('usr_id' => $user->usr_id));
