<?php
  ...
  class dashboarda_model extends CI_Model {
       ...
       function countTotalEnquires() {
            ...
            $analysis['stock_vehicle'] = $this->db->query("SELECT COUNT(*) AS vehicle_count FROM " . $this->tbl_valuation . ' WHERE val_status = 1 AND (val_type = 1 OR val_type = 2)')->row()->vehicle_count;
            ...
            if (is_roo_user()) {
                 ...
                 $where = ' WHERE enq_current_status = 99';
                 $analysis['deleted_vehicles'] = $this->db->query("SELECT COUNT(*) AS enq_total FROM " . $this->tbl_enquiry . $where)->row()->enq_total;
                 ...
                 $where = ' WHERE enq_current_status = 99 AND enq_showroom_id = ' . $showroom;
                 $analysis['deleted_vehicles'] = $this->db->query("SELECT COUNT(*) AS enq_total FROM " . $this->tbl_enquiry . $where)->row()->enq_total;
                 ...
                 $where = ' WHERE enq_current_status = 99 AND enq_added_by = ' . $this->uid;
                 $analysis['deleted_vehicles'] = $this->db->query("SELECT COUNT(*) AS enq_total FROM " . $this->tbl_enquiry . $where)->row()->enq_total;
                 ...
                 $analysis['active_enquires'] = $this->db->where_in($this->tbl_enquiry . '.enq_se_id', $mystaffs)
                    ->where_in($this->tbl_enquiry . '.enq_current_status', $this->myEnqStatuses)->count_all_results($this->tbl_enquiry);
                 
                 //$this->db->query("SELECT COUNT(*) AS enq_total FROM " . $this->tbl_enquiry . $where)->row()->enq_total;

                 $where = ' WHERE enq_current_status = 3 AND enq_se_id IN (' . $mystaffs . ')';
                 $analysis['dropped_vehicles'] = $this->db->query("SELECT COUNT(*) AS enq_total FROM " . $this->tbl_enquiry . $where)->row()->enq_total;

                 $where = ' WHERE enq_current_status = 99 AND enq_se_id IN (' . $mystaffs . ')';
                 $analysis['deleted_vehicles'] = $this->db->query("SELECT COUNT(*) AS enq_total FROM " . $this->tbl_enquiry . $where)->row()->enq_total;

                 $where = ' WHERE enq_current_status = 7 AND enq_se_id IN (' . $mystaffs . ')';
                 $analysis['soled_vehicles'] = $this->db->query("SELECT COUNT(*) AS enq_total FROM " . $this->tbl_enquiry . $where)->row()->enq_total;
       ...
       function dashboardMeterials() {
            ...
            $today = date('Y-m-d') . '<br>';
            $onemonthBefore = date('Y-m-d', (strtotime('-30 day', strtotime(date('Y-m-d')))));
            ...
            if (is_roo_user()) { // root users
                 ...
                 $analysis['mode_of_enq_count'] = $this->db->select("COUNT(*) AS total_no, enq_mode_enq, enq_entry_date")
