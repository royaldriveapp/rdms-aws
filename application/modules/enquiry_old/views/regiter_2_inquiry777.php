...
<script>
    function fetchCustomers(searchType, searchValue) {
    const apiUrl = searchType === 'phone' 
        ? `${site_url}customers/searchCustomerByPhone?search_term=${searchValue}`
        : `${site_url}customers/searchCustomerByName?search_term=${searchValue}`;

    fetch(apiUrl)
        .then(response => response.json())
        .then(data => {
            const dropdown = searchType === 'phone' ? 'searchByPhone' : 'searchByName';
            const select = document.getElementById(dropdown);
            select.innerHTML = '<option value="">Select...</option>';
            data.forEach(customer => {
                select.innerHTML += `<option value='${JSON.stringify(customer)}'>${customer.cusd_name} - ${customer.cup_phone}</option>`;
            });
        });
}
...
function selectCustomer(select) {
    ...
    if (customer) {
        document.getElementById('vreg_cust_id').value = customer.cusd_id;
        document.getElementById('enq_cus_name').value = customer.cusd_name;

        // Update dynamically generated phone fields
        const phoneContainer = document.querySelector('.phone-fields');
        phoneContainer.innerHTML = '';
        customer.cup_phone.split(',').forEach((phone, index) => {
            phoneContainer.innerHTML += `
                <div class="form-group">
                    <label for="enq_cus_mobile_${index + 1}">Mobile${index + 1}</label>
                    <input type="text" class="form-control" id="enq_cus_mobile_${index + 1}" value="${phone}">
                </div>`;
        });
    }
}
...
document.getElementById('searchByPhone').addEventListener('keyup', function () {
    if (this.value.length >= 9) {
        fetchCustomers('phone', this.value);
    }
});

document.getElementById('searchByName').addEventListener('keyup', function () {
    if (this.value.length >= 4) {
        fetchCustomers('name', this.value);
    }
});
                    ...
                    <form id="wizard" data-url="<?php echo site_url('enquiry/punchEnquiry'); ?>" class="form_wizard wizard_horizontal" action="#" role="form" data-toggle="validator" method="post" accept-charset="utf-8">
                        ...
                        <input type="hidden" value="<?php echo isset($datas['vreg_cust_id']) ? $datas['vreg_cust_id'] : ''; ?>" name="vreg_cust_id" />
    ...
    <!-- @Phones -->
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <div class="col-md-6 col-sm-6 col-xs-12">
                                                    <input value="<?php echo isset($datas['cit_name']) ? $datas['cit_name'] : ''; ?>" id="enq_cus_city" class="autoComCity form-control form-control-enq col-md-7 col-xs-12" type="text" name="enquiry[enq_cus_city]">
                                                    </div>
                                                </div>
...
function debouncedSearch() {
    ...
    debounceTimer = setTimeout(searchCustomers, 300); // Debounce API calls
}
...
function searchCustomers() {
    ...
    fetch(`${site_url}customers/searchCustomers?search_term=${encodeURIComponent(searchValue)}`)
        .then(response => response.json())
        .then(data => {
            ...
            if (data.error || data.length === 0) {
                ...
                data.forEach(customer => {
                    resultsContainer.innerHTML += `
                        <div 
                            class="dropdown-item" 
                            onclick="selectCustomer('${customer.cusd_id}', '${customer.cusd_name}', '${customer.cusd_address}', '${customer.cup_phone}')">
                            ${customer.cusd_name} - ${customer.cusd_address} - ${customer.cup_phone}
                        </div>
                    `;
                });
            }
...
function selectCustomer(id, name, address, phone) {
    alert(`Selected: ${name}, ${address}, ${phone}`);
    document.getElementById('vreg_cust_id').value = id;
    document.getElementById('enq_cus_name').value = name;

    // Add phone field dynamically
    const phoneContainer = document.querySelector('.phone-fields');
    phoneContainer.innerHTML = `
        <div class="form-group">
            <label for="enq_cus_mobile">Mobile</label>
            <input type="text" class="form-control" id="enq_cus_mobile" value="${phone}">
        </div>
    `;
