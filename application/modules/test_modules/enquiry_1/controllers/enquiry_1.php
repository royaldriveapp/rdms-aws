<?php
  ...
  class enquiry_1 extends App_Controller {
     ...
     function exportRegisterReportExl() {
          ...
          generate_log(array(
              'log_title' => $this->session->userdata('usr_username') . ' downloaded excel report for my register report',
              'log_desc' => $this->session->userdata('usr_username') . ' downloaded excel report for my register report on - ' . date('Y-m-d H:i:s'),
          ...
          $heading = array('Entry date', 'Customer name', 'Contact', 'Location', 'Disctrict', 'Contact mode', 'Brand', 'Model', 'Variant',
              'Year', 'Investment', 'Call type', 'Department', 'Added by', 'Assigned to', 'Remarks', 'Sales officer comment', 'Cust status');
          ...
          if (isset($datas['data']) && !empty($datas['data'])) {
               foreach ($datas['data'] as $key => $value) {
                    ...
                    $regSts = isset($sts[$value['vreg_customer_status']]) ? $sts[$value['vreg_customer_status']] : '';
                    ...
                    $objPHPExcel->getActiveSheet()->setCellValue('R' . $row, $regSts);
                    $row++;
                    $no++;
               }
          }
...
//                 }
                 if (empty($message)) {
		            ...
		            $applink = get_logged_user('usr_appdownloadlink');
                      $applink = !empty($applink) ? $applink : 'https://bit.ly/3tQPhvL';
...
My self $myname, Please feel free to contact me @ $mynumber. Regards, $myname admin@royaldrive.in, https://royaldrive.in App Link: " . $applink;
       ...
       function regiter_2_inquiry($id) {
            ...
            $data['questions'] = $this->enquiry->getInquiryQuestions();
            $data['brands'] = $this->enquiry->getBrands();
            $data['evaluation'] = $this->evaluation->getAllStock();
            $data['salesExe'] = $this->emp_details->salesExecutives();
...
//                 }
                 if (empty($message)) {
                      ...
                      $mynumber = get_logged_user('usr_phone');
                      $mycustmr = trim($_POST['enquiry']['enq_cus_name']);
                      $mobile_number = str_replace(' ', '', $_POST['enquiry']['enq_cus_mobile']);
                      /*$message = "Hi Mr.$mycustmr
Greetings from Royal Drive South India's Largest Pre-Owned Luxury Car Showroom Thank you for the visit, We are grateful for serving you. 
My self $myname, Please feel free to contact me @ $mynumber. Regards, $myname admin@royaldrive.in https://royaldrive.in App Link: http://onelink.to/p4z8q2";*/

                      $message = "Dear " . $mycustmr . ", my self " . $myname . " Please feel free to contact me on " . $mynumber . " - Royal Drive South India's Largest Pre-Owned Luxury Car Showroom";
                      send_sms($message, $mobile_number , 'new enquiry punched', '1607100000000042909');
     ...
     public function courtesyCallList() {
          $data['data'] = $this->enquiry->courtesyCallList();
          $this->render_page(strtolower(__CLASS__) . '/courtesyCallList', $data);
     }
} 