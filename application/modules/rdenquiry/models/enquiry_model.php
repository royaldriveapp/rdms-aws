...
//                    $where[$this->tbl_enquiry . '.enq_current_status !='] = 9;
                      ...
                      if(check_permission('enquiry', 'enq_showsalesonly')) {
                         $this->db->where($this->tbl_enquiry . '.enq_cus_status', 1);
                      } else if(check_permission('enquiry', 'enq_showpuchsonly')) {
                         $this->db->where('(' . $this->tbl_enquiry . '.enq_cus_status = 2 OR ' . $this->tbl_enquiry . '.enq_cus_status = 3)');
                      }
                      if(check_permission('enquiry', 'showonlymyself')) {
                         $where[$this->tbl_enquiry . '.enq_se_id'] = $this->uid;
                      }  
                      if(check_permission('enquiry', 'showmyshowroom')) {
...
//                           $answer = array_values($value)[0];
       ...
       function permenentDelete($enqId) {
            if (!empty($enqId)) {
                 ...
                 $this->db->delete($this->tbl_vehicle_status, array('vst_enq_id' => $enqId));
       ...
       function getTrackCardDetails($enqId) {
            ...
            if (!empty($enq)) {
                 ...
                 $enq['regAssigned'] = $this->db->select($this->tbl_register_master . '.vreg_id, vreg_first_added_on, ' . $this->tbl_users . '.usr_username')
                                          ->join($this->tbl_users, $this->tbl_users . '.usr_id = ' . $this->tbl_register_master . '.vreg_first_owner', 'LEFT')
                                          ->where($this->tbl_register_master . '.vreg_inquiry', $enqId)->order_by('vreg_id', 'DESC')
                                          ->get($this->tbl_register_master)->result_array();

                                          if($this->uid == 100) {
                                               //debug($enq['regAssigned']);
                                          }
            }
            return $enq;
       }
...
//                            ->where_in($this->tbl_enquiry . '.enq_current_status', $this->myEnqStatuses)
     ...
     function reassignenquiry($data) {
          ...
          $this->db->where('enq_id', $data['enq_id'])->update($this->tbl_enquiry, array(
               'enq_se_id' => $data['new_se_id'], 'enq_division' => $newDivision, 'enq_showroom_id' => $newShowroom , 'enq_current_status_history' => $historyId
          ));
