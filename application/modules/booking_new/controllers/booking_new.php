...
class booking_new extends App_Controller
{
     ...
     function canceltoken($bkId)
     {
          if (!empty($bkId)) {
               $data['banks'] = $this->evaluation->getAllBanks();
               $data['data'] = $this->booking->BookingDetailsPushsales($bkId);
               $data['__CLASS__'] = __CLASS__;
               $this->render_page(strtolower(__CLASS__) . '/canceltoken', $data);
          }
     }
}
