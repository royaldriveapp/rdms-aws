                ...
                <div class="x_content">
                    <form class="frmFilter" method="post">
                        <table>
                            <tr>
                                <td>
                                    <input autocomplete="off" name="vbk_added_on_from" type="text" class="dtpEnquiry form-control col-md-7 col-xs-12" placeholder="Booking date from" value="<?php echo isset($_GET['vbk_added_on_from']) ? $_GET['vbk_added_on_from'] : ''; ?>" />
                                </td>
                                <td style="padding-left: 10px;">
                                    <input autocomplete="off" name="vbk_added_on_to" type="text" class="dtpEnquiry form-control col-md-7 col-xs-12" placeholder="Booking date to" value="<?php echo isset($_GET['vbk_added_on_to']) ? $_GET['vbk_added_on_to'] : ''; ?>" />
                                </td>
                                ...
                                <td>
                                    <select multiple="multiple" style="float: left;width: auto;" class="cmbMultiSelect select2_group form-control" name="status[]">
                                        <option value="<?php echo confm_book; ?>" <?php echo (@in_array(confm_book, $sts)) ? 'selected="selected"' : ''; ?>>
                                            Booking confirmed</option>

                                        <option value="<?php echo vehicle_booked; ?>" <?php echo (@in_array(vehicle_booked, $sts)) ? 'selected="selected"' : ''; ?>>
                                            Vehicle booked</option>

                                        <option value="<?php echo book_delvry; ?>" <?php echo (@in_array(book_delvry, $sts)) ? 'selected="selected"' : ''; ?>>
                                            Vehicle delivered</option>

                                        <option value="<?php echo 29; ?>" <?php echo (@in_array(29, $sts)) ? 'selected="selected"' : ''; ?>>Booking
                                            cancel</option>
                                    </select>
                                </td>
                                <?php if (check_permission('booking', 'showall')) { ?>
                                    <td>
                                        <select style="float: left;width: auto;" class="cmbMultiSelect select2_group form-control" name="showroom">
                    ...
                    <table id="tblBooking" class="table table-striped table-bordered display nowrap" style="width:100%;white-space: nowrap;">
                        <thead>
                            <tr>
                                ...
                                <th>Token Approved by</th>
                                <th>Sales Approved by</th>
                                <?php if ($this->uid == 100) : ?>
                                    <th>Delete</th>
                                    <th>Del Sls tkn</th>
                                    <th>Del Sls</th>
                                ...
                                if (check_permission('booking', 'editbooking')) : ?>
                                    <th>Edit</th>
                                <?php endif;
                                if (check_permission('booking', 'canapprovetokenforihits')) : ?>
                                    <th>Approve token</th>
                                <?php endif;
                                if (check_permission('booking', 'canapprovebookingforihits')) : ?>
                                    <th>Approve sales</th>
                                <?php endif;
                                if (check_permission('booking', 'canceltoken')) : ?>
                                    <th>Cancel Token</th>
                                <?php endif;
                                if (check_permission('booking', 'canceltoken')) : ?>
                                    <th>Cancel Sales</th>
...
<script>
    ...
    function empList(canDelete, frmData) {
        ...
        var darTable = $('#tblBooking').DataTable({
            ...
            "serverSide": true,
            "serverMethod": "post",
            "ajax": site_url + "booking_new/index_ajax?" + frmData,
            "columns": [{
                    "data": "vbk_ref_no"
                }, // Booking ID
                {
                ...
                {
                    "data": "tknapdby_username"
                },
                {
                    "data": "slsapdby_username"
                },
                <?php if ($this->uid == 100) : ?> {
                        "render": function(data, type, full, meta) {
                            ...
                            var content = '<a class="btnRemoveTableRow" href="javascript:void(0);" data-url="' +
                                deleteUrl +
                                '"><i style="color: #fff;" title="View document" class="fa fa-trash"></i></a>';
                            return content;
                        }
                    },
                    {
                        "render": function(data, type, full, meta) {
                            var deleteUrl = '<?php echo site_url("booking/deletesales/token"); ?>/' + full.vbk_id;
                            var content = '<a href="' + deleteUrl + '"><i style="color: #fff;" title="Delete Sales Token" class="fa fa-trash"></i></a>';
                            return content;
                        }
                    },
                    {
                        "render": function(data, type, full, meta) {
                            var deleteUrl = '<?php echo site_url("booking/deletesales/sales"); ?>/' + full.vbk_id;
                            var content = '<a href="' + deleteUrl + '"><i style="color: #fff;" title="Delete Sales" class="fa fa-trash"></i></a>';
                            return content;
                        }
                    },
                <?php endif;
                if (check_permission('booking', 'editbooking')) : ?> {
                        "render": function(data, type, full, meta) {
                            ...
                            var content = '<a href="' + editUrl +
                                '"><i style="color: #fff;" title="Update booking" class="fa fa-pencil"></i></a>';
                            return content;
                        }
                    },
                <?php endif;
                if (check_permission('booking', 'canapprovetokenforihits')) : ?> {
                        "render": function(data, type, full, meta) {
                            if (parseInt(full.vbk_token_approve_by) > 0) {
                                return 'Approved';
                            } else { //if (full.vbk_status != book_delvry)
                                var deleteUrl = '<?php echo site_url("booking/pushsalestokenpreview"); ?>/' +
                                    full
                                    .vbk_id;
                                return '<a href="' + deleteUrl + '">Approve</a>';
                            }
                            return '';
                        }
                    },
                ...
                if (check_permission('booking', 'canapprovebookingforihits')) : ?> {
                        "render": function(data, type, full, meta) {
                            if (parseInt(full.vbk_token_approve_by) == 0) {
                                return 'Token not approved';
                            } else if (parseInt(full.vbk_sales_approve_by) > 0) {
                                return 'Approved';
                            } else { //if (full.vbk_status != book_delvry)
                                var deleteUrl = '<?php echo site_url("booking/pushsalespreview"); ?>/' +
                                    full
                                    .vbk_id;
                                return '<a href="' + deleteUrl + '">Approve</a>';
                            }
                ...
                if (check_permission('booking', 'canceltoken')) : ?> {
                        "render": function(data, type, full, meta) {
                            var deleteUrl = '<?php echo site_url("booking_new/canceltoken"); ?>/' + full.vbk_id;
                            return '<a href="' + deleteUrl + '">Cancel Token</a>';
                        }
                    },
                <?php endif;
                if (check_permission('booking', 'cancelbooking')) : ?> {
                        "render": function(data, type, full, meta) {
                            var deleteUrl = '<?php echo site_url("booking_new/cancelbooking"); ?>/' + full.vbk_id;
                            return '<a href="' + deleteUrl + '">Cancel Booking</a>';
                        }
                    }
                <?php endif; ?>
            ],
