...
class booking_model extends CI_Model
{
     ...
     function getDivision($isSales = false)
     {
          if ($isSales) {
               $this->db->where('div_id = 1')->or_where('div_id = 2');
          }
     ...
     function getAllBookings($filter = '')
     {
          ...
          $selectArray = array(
               ...
               'bkdby.usr_last_name AS btdby_last_name',
     ...
     function reserveVehicle($data)
     {
          ...
          if (!empty($showroom)) {
               ...
               $vehicleName = '';
               if (!empty($vehicle)) {
                    $regNo = $vehicle['val_veh_no'];
                    unset($vehicle['val_veh_no']);
                    ...
                    if (!empty($cdo)) {
                         $perNumber = isset($data['bm']['vbk_per_ph_no']) ? $data['bm']['vbk_per_ph_no'] : '';
                         $offNumber = isset($data['bm']['vbk_off_ph_no']) ? $data['bm']['vbk_off_ph_no'] : '';
                         $toNumber = !empty($perNumber) ? str_replace(' ', '', $perNumber) : str_replace(' ', '', $offNumber);
                         ...
                         $apiData = array(
                              ...
                              "campaignName" => "vb_sales_booking",
                              'destination' => $toNumber,
                              'userName' => 'royaldrive9090@gmail.com',
                              'templateParams' => array($vehicleName, $regNo, '7', '+' . $cdo['usr_did_number'], $cdo['usr_username'] . '\n' . $cdo['desig_title'])
                         );
     ...
     function getBookedVehicle($bookid = 0, $stsId = 0)
     {
          ...
          $selectArray = array(
               ...
               'bkdby.usr_first_name AS bkdby_first_name',
               'bkdby.usr_last_name AS btdby_last_name',
               'bkdby.usr_username AS bkdby_username',
               $this->tbl_valuation . '.val_veh_no',
          ...
          if ($bookid > 0) {
               ...
               $hisArray = array(
                    ...
                    'bkdby.usr_first_name AS bkdby_first_name',
                    'bkdby.usr_last_name AS btdby_last_name',
                    'addby.usr_first_name AS addby_first_name',
                    'addby.usr_last_name AS addby_last_name',
          ...
          if (!empty($data)) {
               ...
               if (!isset($data['confim']['vbc_verify_by_grp_slug']) && check_permission('booking', 'bookingdetailsforrfi')) { // RFI
                    ...
                    $this->db->where('enq_id', $data['confim']['vbc_enq_id'])->update(
                         $this->tbl_enquiry,
                         array(
                              'enq_current_status' => $data['status'],
                              'enq_current_status_history' => $hisId
                         )
                    );
                    ...
                    $this->db->where('enq_id', $data['confim']['vbc_enq_id'])->update(
                         $this->tbl_enquiry,
                         array(
                              'enq_current_status' => $enqCurrentStatus,
                              'enq_current_status_history' => $hisId
                         )
                    );
                    ...
                    if (isset($data['status']) && $data['status'] == confm_book) {
                         ...
                         /*$this->load->model('purchase/purchase_model', 'purchase');
                        $cdo = $this->purchase->getCustomerDelightStaff();
                        if(!empty($cdo) && isset($data['vehicleDetails'])) {
                            $vehiclePostData = unserialize($data['vehicleDetails']);
                            if(!empty($vehiclePostData)) {
                                $perNumber = isset($vehiclePostData['vbk_per_ph_no']) ? $vehiclePostData['vbk_per_ph_no'] : '';
                                $offNumber = isset($vehiclePostData['vbk_off_ph_no']) ? $vehiclePostData['vbk_off_ph_no'] : '';
                                $toNumber = !empty($perNumber) ? str_replace(' ', '', $perNumber) : str_replace(' ', '', $offNumber);
                                $vehicleName = $vehiclePostData['brd_title'] . ', ' . $vehiclePostData['mod_title'] . ', ' . $vehiclePostData['var_variant_name'];
                                $regNo = $vehiclePostData['val_veh_no'];
                                
                                $apiData = array(
                                    "apiKey" => "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjY1MDJmY2ZiOTQzM2U4MGJjZTQ5OGE4MyIsIm5hbWUiOiJST1lBTERSSVZFIFBSRSBPV05FRCBDQVJTIExMUCIsImFwcE5hbWUiOiJBaVNlbnN5IiwiY2xpZW50SWQiOiI2NTAyZmNmYjk0MzNlODBiY2U0OThhN2UiLCJhY3RpdmVQbGFuIjoiQkFTSUNfTU9OVEhMWSIsImlhdCI6MTY5NDY5NDY1MX0.EfRR_D4lFORcRC4rkGr9xglt21CP412EDVTRPOFxuYc",
                                    "campaignName" => "vb_sales_booking",
                                    'destination' => '919745661946',//$toNumber,
                                    'userName' => 'royaldrive9090@gmail.com',
            			            'templateParams' => array($vehicleName, $regNo, '7', '+' . $cdo['usr_did_number'], $cdo['usr_username'] .'\n' . $cdo['desig_title'])
                                );
                                $data_string = json_encode($apiData);
     ...
     function getAllDeliveryPaginate($postDatas, $filter)
     {
          //debug($filter);
          $draw = $postDatas['draw'];
          $row = $postDatas['start'];
          ...
          $totalRecords = $this->getDeliveryTotal($postDatas, $filter);
          ...
          if (isset($filter['division']) && !empty($filter['division'])) {
               $this->db->where($this->tbl_divisions . '.div_id', $filter['division']);
          }
          if (isset($filter['vbk_added_on_from']) && !empty($filter['vbk_added_on_from'])) {
               $date = date('Y-m-d', strtotime($filter['vbk_added_on_from']));
               $this->db->where('DATE(' . $this->tbl_vehicle_booking_master . '.vbk_added_on) >=', $date);
          ...
          $this->db->where($this->tbl_statuses . '.sts_value', book_delvry);
     ...
     function getDeliveryTotal($postDatas, $filter)
     {
          $searchValue = $postDatas['search']['value']; // Search value
          if (check_permission('booking', 'showall')) {
          } else if (check_permission('booking', 'mybking')) {
