                    ...
                    <ul class="nav navbar-right panel_toolbox">
                        <?php if (check_permission('booking', 'exportbookedvehicles')) { ?>
                            <li style="float: right;">
                                <a class="aDownloadPdf"
                                    href="<?php echo site_url('booking/exportbookedvehicles?status=40&' . $_SERVER['QUERY_STRING']); ?>">
                    ...
                    <form class="frmFilter" method="post">
                        <table>
                            <tr>
                                ...
                                <?php } ?>
                                <td>
                                    <select style="float: left;width: auto;" class="form-control" name="division">
                                        <option value="0">All division</option>
                                        <?php foreach ((array) $division as $key => $value) { ?>
                        ...
                        <thead>
                            <tr>
                                ...
                                if (check_permission('booking', 'canapprovetokenforihits')) : ?>
                                    <th>Approve token</th>
                                <?php endif; ?>
                                <?php if (check_permission('booking', 'canapprovebookingforihits')) : ?>
                                    <th>Approve sales</th>
                                <?php endif; ?>
                            </tr>
                        </thead>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $(document).on('submit', '.frmFilter', function(e) {
            e.preventDefault();
            $('.aDownloadPdf').attr('href', 'https://rdms.royaldrive.in/booking/exportbookedvehicles?status=40&' + $(
                this).serialize());
    ...
    function empList(canDelete, frmData) {
        ...
        var darTable = $('#tblBooking').DataTable({
            ...
            "serverSide": true,
            "serverMethod": "post",
            "ajax": site_url + "booking/deliverdvehicleAjax?" + frmData,
            "columns": [{
                    "data": "vbk_ref_no"
                }, // Booking ID
                {
                ...
                }, // Expect delivery on
                {
                    ...
                    "sortable": true,
                    "render": function(data, type, full, meta) {
                        var vbk_delivery_date = (full.vbk_delivery_date !== null && full
                            .vbk_delivery_date !== '') ? new Date(full.vbk_delivery_date) : null;
                        if (vbk_delivery_date) {
                            return vbk_delivery_date.toLocaleString('en-US', {
                                day: 'numeric',
                                month: 'short',
                                year: 'numeric',
                                hour: '2-digit',
                ...
                <?php if ($this->uid == 100) : ?> {
                    ...
                    {
                        "render": function(data, type, full, meta) {
                            var deleteUrl = '<?php echo site_url("booking/deletesales/token"); ?>/' + full
                                .vbk_id;
                            var content = '<a href="' + deleteUrl +
                                '"><i style="color: #fff;" title="Delete Sales Token" class="fa fa-trash"></i></a>';
                            return content;
                        }
                    },
                    {
                        "render": function(data, type, full, meta) {
                            var deleteUrl = '<?php echo site_url("booking/deletesales/sales"); ?>/' + full
                                .vbk_id;
                            var content = '<a href="' + deleteUrl +
                                '"><i style="color: #fff;" title="Delete Sales" class="fa fa-trash"></i></a>';
                            return content;
                        }
                    },
                <?php endif;
