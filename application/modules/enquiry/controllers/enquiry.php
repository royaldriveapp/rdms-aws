...
class enquiry extends App_Controller
{
     ...
     function exportExcelStatus($status)
     {
          ...
          $lostDropReason = unserialize(lostDrop);
          if (!empty($enquires['data'])) {
               foreach ($enquires['data'] as $key => $value) {
                    ...
                    $reson = isset($lostDropReason[$value['enh_reason_for_lost_drop']]) ? $lostDropReason[$value['enh_reason_for_lost_drop']] : '';
                    $otherReson = (isset($value['enh_reason_for_lost_drop_other']) && !empty($value['enh_reason_for_lost_drop_other'])) ? ' (' . $value['enh_reason_for_lost_drop_other'] . ')' : '';
                    $objPHPExcel->getActiveSheet()->setCellValue('Y' . $row, $reson . $otherReson);
                    $row++;
                    $no++;
               }
          }
...
// debug($enquires);
     ...
     public function add()
     {
          if (!empty($_POST)) {
               ...
               if (empty($message)) {
                    ...
                    $message = "Dear $mycustmr Greetings from Royal Drive South India's Largest Pre-Owned Luxury Car Showroom, We thank-you for spending your valuable time to us. We are more happy to help you. My self $myname Please feel free to contact me @ $mynumber Regards, $myname www.royaldrive.in App Link: $applink Royal Drive South India's Largest Pre-Owned LLP";

                    if ($this->enquiry->newEnquiry($_POST)) {
                         if ($mobile_number = is_indian_number($_POST['enquiry']['enq_cus_mobile'])) {
                              //send_trans_sms($message, $mobile_number, 'sms-enquiry', '1607100000000291697');
                         } else if ($mobile_number = is_indian_number($_POST['enquiry']['enq_cus_whatsapp'])) {
                              //send_trans_sms($message, $mobile_number, 'sms-enquiry', '1607100000000291697');
                         }
                         //send_trans_sms($message, '919745661946', 'sms-enquiry', '1607100000000291697');
     ...
     function view_change($enq_id, $vreg_department = '', $reg_id = '')
     {
          if (!empty($enq_id)) {
               ...
               // debug($data['brands'] );
               //debug(464);
               $data['enquiry'] = $this->enquiry->enquires($enq_id);//mn qry
               if ($this->uid == 100) {
                    //debug($data['enquiry']);
               }
               //  debug($data['enquiry']);
          ...
          if (!empty($enq)) {
               ...
               if ($status == 8) {
                    $data['statusButtons'] = array(
                         ...
                         array('id' => 99, 'title' => 'Delete', 'buttonClass' => 'success btnValidate')
                    );
               } else if ($status == 2) {
                    $data['statusButtons'] = array(
                         array('id' => 1, 'title' => 'Cancel Request', 'buttonClass' => 'danger'),
                         array('id' => 3, 'title' => 'Drop', 'buttonClass' => 'success btnValidate')
                    );
               } else if ($status == 6) {
                    $data['statusButtons'] = array(
                         array('id' => 1, 'title' => 'Cancel Request', 'buttonClass' => 'danger'),
                         array('id' => 7, 'title' => 'Close', 'buttonClass' => 'success btnValidate')
                    );
               } else if ($status == 4) {
                    $data['statusButtons'] = array(
                         array('id' => 5, 'title' => 'Lost confirmed', 'buttonClass' => 'danger btnValidate'),
     ...
     function myregister($test = '')
     {
       ...
       // $enquires = $this->enquiry->readVehicleReg(0, $limit, $page, $_GET);//og old
    $enquires = $this->enquiry->readVehicleRegNew(0, $limit, $page, $_GET);//count
    // debug($enquires);
         // debug($_GET);
          //$end = microtime(true);
          //$exec_time = ($end - $start);
          ...
          $data['teleCallers'] = $this->emp_details->teleCallersSalesStaffs();
         // $data['customer_phones'] = $this->customers->getCustomerPhonesByCustomerId($data['datas']['vreg_cust_id']);
          //  $end = microtime(true);
          //$exec_time = ($end - $start);
          //echo "The execution time of the PHP script is : ".$exec_time." sec";
          //debug($data);
     ...
     function regiter_2_inquiry($id)
     {
          ...
          //$data['datas'] = $this->enquiry->readVehicleReg($id);//mn qry---->
          $data['datas'] = $this->enquiry->readVehicleRegNew($id);//mn qry---->
         //debug($data['datas']);
          $data['customer_phones'] = $this->customers->getCustomerPhonesByCustomerId($data['datas']['vreg_cust_id']);
//debug($data['customer_phones']);
          //  debug(   $data['datas'] );
          $data['Profession'] = $this->enquiry->getProfession();
     ...
     public function punchEnquiry()
     {
       ...
       // debug($_POST,1); exit;
          ...
          // exit;
          //  if($this->uid==358 OR $this->uid==100){
              // $message="hgfjhdgf";
             //  $mobile_number=919656557105;
        /// echo send_sms($message, $mobile_number, 'sms-enquiry');
...
My self $myname, Please feel free to contact me @ $mynumber. Regards, $myname info@royaldrive.in www.royaldrive.in App Link: http://onelink.to/p4z8q2";
                    if ($enquiryId = $this->enquiry->newEnquiry($_POST, $valId)) { //$valId == cpnl_valuation table master id
                         ...
                         $hid=$this->enquiry->addEnquiryHistory(
                              array(
                                   'enh_status' => 1,
...
//echo $_POST['customer_phones']['0']; exit;
                         if ($mobile_number = is_indian_number($_POST['customer_phones']['0'])) {
     ...
     function bindShowroomByDivision()
     {
        ...
        //  header('Content-Encoding: identity'); // avd comprssion issues
     ...
     function storeJsonOld($data)
     {


          // Specify the file path to the JSON data
          $filePath = '../rdms/jsonDB/enq_data.json';

          // Load existing JSON data from the file
          $existingData = file_exists($filePath) ? file_get_contents($filePath) : '{}';
          $existingData = json_decode($existingData, true);
          // Initialize ID counter if it doesn't exist
          if (!isset($existingData['id_counter'])) {
               $existingData['id_counter'] = 0;
          }

          // Increment ID counter for the new record
          $newId = ++$existingData['id_counter'];

          // Add the new ID to the new record data
          $data['id'] = $newId;

          // Add the new ID to the new record data
          //$data['id'] = $existingData['id_counter'];
          // Remove unnecessary keys
          unset($data['veh_stock_id']); // Remove veh_stock_id key

          // Extract vehicle sale data
          $vehicleSaleData = $data['vehicle']['sale'];
          unset($data['vehicle']); // Remove vehicle key

          // Combine the data with vehicle sale information
          $data['enquiry']['enq_vehicle_sales'] = [];
          foreach ($vehicleSaleData['veh_brand'] as $key => $value) {
               $saleRecord = [];
               foreach ($vehicleSaleData as $field => $values) {
                    $saleRecord[$field] = $values[$key];
               }
               $data['enquiry']['enq_vehicle_sales'][] = $saleRecord;
          }
          ...
          echo "Data has been successfully stored in the JSON file.";
     }
     ...
     function updateJsonRecord($id, $newData)
     {
          // Specify the file path to the JSON data
          $filePath = '../rdms/jsonDB/enq_data.json';
          ...
          foreach ($existingData as $key => &$record) {
               if (isset($record['vreg_id']) && $record['vreg_id'] == $id) {
          ...
          // Output success message
          echo "Record with ID $id has been successfully updated.";
     }

     function countRecordsByFiltersOld($enq_se_id, $currentMonth)
     {
          // Specify the file path to the JSON data
          $filePath = '../rdms/jsonDB/enq_data.json';
          ...
          foreach ($dataArray as $record) {
               // Check if enq_se_id matches
               // and if enq_entry_date is within the current month
               if ($record['enquiry']['enq_se_id'] == $enq_se_id) {
                    $entryDate = date_create_from_format('d-m-Y', $record['enquiry']['enq_entry_date']);
     ...
     function countRecordsByFilters($enq_se_id, $currentMonth)
     {
          // Specify the file path to the JSON data
          $filePath = '../rdms/jsonDB/enq_data.json';
          ...
          // Iterate through each record in the array
          foreach ($dataArray as $record) {
               // Check if the record has the 'enquiry' key
               if (isset($record['enquiry'])) {
                    $enquiry = $record['enquiry'];
                    // Check if enq_se_id matches and enq_entry_date is within the current month
                    if ($enquiry['enq_se_id'] == $enq_se_id) {
                         $entryDate = date_create_from_format('d-m-Y', $enquiry['enq_entry_date']);
                         if ($entryDate !== false) {
                              $entryMonth = (int)$entryDate->format('m');
                              if ($entryMonth == $currentMonth) {
                                   $count++;
                              ...
                              echo "Error parsing date: " . $enquiry['enq_entry_date'];
                         }
                    }
               }
          }
     ...
     function q()
     {
          ...
          $count = $this->countRecordsByFilters($enq_se_id, $currentMonth);
          echo "Count of records where enq_se_id is $enq_se_id and enq_entry_date is within the current month: $count";

          exit;
          $updateData = [
               'enquiry' => [
                    'enq_cus_name' => 'Updated Name',
                    'enq_cus_mobile' => '10076543210',
                    // Add more fields to update as needed
               ],
               // Add more keys and data to update as needed
          ];
          $this->updateJsonRecord('233049', $updateData);


          $query = $this->db->get('cpnl_vehicle');
          $j = $query->row_array(); // 
          debug($j);
     }
}
