...
class enquiry_model extends CI_Model
{
     ...
     private function selectArrayEnq()
{
    return [
    ...
    $this->tbl_customer_details . '.cusd_whatsapp',
     ...
     private function joinEnq()
     {
          $this->db->join($this->tbl_users, $this->tbl_users . '.usr_id = ' . $this->tbl_enquiry . '.enq_se_id', 'left');
          $this->db->join($this->tbl_users . ' tbl_added_by', 'tbl_added_by.usr_id = ' . $this->tbl_enquiry . '.enq_added_by', 'left');
          $this->db->join($this->tbl_statuses, $this->tbl_enquiry . '.enq_current_status = ' . $this->tbl_statuses . '.sts_value_new', 'left');
          $this->db->join($this->tbl_customer_details, $this->tbl_customer_details . '.cusd_id = ' . $this->tbl_enquiry . '.enq_cus_id', 'LEFT');
     ...
     { //enquires_rl
          ...
          if (!empty($id)) {
               ...
               $whereSearch = '';
               if (isset($exParams['search']) && !empty($exParams['search'])) {//new
                    $exParams['search'] = trim($exParams['search']);
                    $whereSearch = '(' . $this->tbl_customer_details . ".cusd_name LIKE '%" . $exParams['search'] . "%' OR " .
                         $this->tbl_users . ".usr_first_name LIKE '%" . $exParams['search'] . "%' OR " .
                         $this->tbl_customer_phones . ".cup_phone LIKE '%" . $exParams['search'] . "%' OR " .
                         $this->tbl_enquiry . ".enq_number LIKE '%" . $exParams['search'] . "%' OR " .
                         $this->tbl_customer_details . ".cusd_whatsapp LIKE '%" . $exParams['search'] . "%' )";
               }
               if (is_roo_user()) {
                    if (!empty($whereSearch)) {
                         ...
                         $this->joinEnq();
                         $this->db->select('COUNT(DISTINCT ' . $this->tbl_enquiry . '.enq_id) as count');
                         $query = $this->db->get($this->tbl_enquiry);
                    ...
                    $selectArrayEnq = $this->selectArrayEnq();
                     $this->db->select(array_merge($selectArrayEnq, [
                         "GROUP_CONCAT(DISTINCT " . $this->tbl_customer_phones . ".cup_phone SEPARATOR ', ') AS phone_numbers"
                    ]));
                  $this->db->from($this->tbl_enquiry);
                  $this->joinEnq();
   $this->db->group_by($this->tbl_enquiry . '.enq_id')->order_by($this->tbl_enquiry . '.enq_id', 'DESC');
   $query = $this->db->get();
...
$this->joinEnq();
$this->db->where($where); 
$this->db->group_by($this->tbl_enquiry . '.enq_id');
$this->db->order_by($this->tbl_enquiry . '.enq_id', 'DESC');
...
$return['data'] = $query->result_array();
  /*@new*/
                   ...
                   /* $return['count'] = $this->db->select($this->tbl_enquiry . '.enq_id,' . $this->tbl_users . '.usr_id, tbl_added_by.usr_username AS enq_added_by_name')
                         ...
                         // avoid NULL issues
                         $return['count'] = isset($result['count']) ? (int) $result['count'] : 0;
                           /*@new*/
                    //Data
                    if (!empty($whereSearch)) {
                         $this->db->where($whereSearch);
...
/*@new*/
 ...
 $return['count'] = isset($result['count']) ? (int) $result['count'] : 0;
  /*@new*/
     ...
     { //2025
         //echo  generate_vehicle_virtual_id(77); exit;
          //print_r($data); exit;
          ...
          if (!empty($data)) {
              ...
              $data['enquiry']['enq_cus_occu_category'] = isset($data['enquiry']['enq_cus_occu_category']) ? $data['enquiry']['enq_cus_occu_category'] : '';
               ...
               $data['enquiry']['enq_cus_mobile'] = $data['customer_phones'][0];
               $enq_place=isset($data['enquiry']['enq_cus_city']) ? $data['enquiry']['enq_cus_city'] : '';
     ...
     /**-------------------------------2025-New2 --------------------------*/
     private function selectVregArray()
{
    return [
...
private function vregSearch($search) {
     return "(" . $this->tbl_customer_details . ".cusd_name LIKE '%" . $this->db->escape_like_str($search) . "%' ESCAPE '!' OR " .
            $this->tbl_customer_details . ".cusd_place LIKE '%" . $this->db->escape_like_str($search) . "%' ESCAPE '!' OR " .
            $this->tbl_customer_phones . ".cup_phone LIKE '%" . $this->db->escape_like_str($search) . "%' ESCAPE '!')";
 }
     ...
     { 
          ...
          if (!$assign) {
               if (!empty($filter)) {
                    ...
                    // Apply search filter
                    if (!empty($search)) {  
                         $whereSearch = $this->vregSearch($search);  
                         $this->db->where($whereSearch, NULL, FALSE);  // Apply raw WHERE condition
                     }
                    if ($brd > 0) {
   ...
   $this->joinTables();
               ...
               $return['count'] = $query->row()->count;
               //select//
               $selectArray = $this->selectVregArray();
