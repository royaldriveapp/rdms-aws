...
$(document).ready(function () {
    ...
    $(document).on('submit', ".calcGrid", function (e) {
        e.preventDefault();
        var url = $(this).attr('action');
        var formData = new FormData($(this)[0]);
        $.ajax({
            ...
            success: function (resp) {
 // Check if grdtl_price is undefined or null
 var price = resp['grid_data']['grdtl_price'];
 var priceElement = document.getElementById("price");
 if (price === undefined || price === null) {
     priceElement.innerHTML = "No data";
 } else {
     priceElement.innerHTML = price;
 }

 // Check if grdm_year_range is undefined or null
 var years = resp['grid_data']['grdm_year_range'];
 var yearsElement = document.getElementById("years");
 if (years === undefined || years === null) {
     yearsElement.innerHTML = "No data";
 } else {
     yearsElement.innerHTML = '<small>'+years+'</small>';
 }
    ...
    $(document).on('change', '.cmbDepartment', function () {
        var id = $('.cmbShowroom').val();
        var dept = $('.cmbDepartment').val();
    ...
    /*Existing*/
    $(document).on('change', '.cmbBindModelExisting', function () {
        ...
        $.ajax({
            ...
            success: function (resp) {
                var model = '<select required="true" style="width: 170px;" class="select2_group form-control cmbBindVariantExisting" \n\
                   name="vehicle[existing][veh_model][]" data-url="' + site_url + 'enquiry/bindVarient"><option value="0">Select Model</option>';
                $.each(resp, function (index, value) {
    ...
    $(document).on('change', '.cmbBindVariantExisting', function () {
        $(this).css('border', '1px solid #ccc');
        var id = $(this).val();
        var url = $(this).attr('data-url');
        var $this = $(this);
        $.ajax({
            ...
            success: function (resp) {
                var variant = '<select style="width: 170px;" class="select2_group form-control" name="vehicle[existing][veh_varient][]"><option value="0">Select Variant</option>';
                $.each(resp, function (index, value) {
    ...
    function calculateVehiclePrice() {
        //
        var ttlAmt = parseInt($('.txtVehiclePrice').val()) + parseInt($('.txtTCSAmt').val()) +
            parseInt($('.txtRTOCharges').val()) + parseInt($('.txtAddRefurbTotal').val())
            + parseInt($('.txtAddAccessoriesTotal').val()) + parseInt($('.txtGrndInsurance').val())
            + parseInt($('.txtBlAmt').val());
        
        $('.txtTtlSaleAmt').val(ttlAmt);
        $('.txtBalanceAmt').val(ttlAmt - parseInt($('.txtAdvanceAmt').val()));
    }
    ...
    function calculateTCS() {
        ...
        var tcs = 0;
        if (parseInt(vehiclePrice) >= parseInt(TCSLimit)) {
    ...
    //     var tmp = '<tr><td></td>' +
    //         '<td><input autocomplete="off" type="text" name="newRefrubishjob[refurb_job][]" class="form-control col-md-7 col-xs-12"/></td>' +
    //         '<td><input autocomplete="off" type="text" name="newRefrubishjob[refurb_job_cost][]" class="decimalOnly form-control col-md-7 col-xs-12"/></td>' +
    //         '<td><input autocomplete="off" type="text" name="newRefrubishjob[actual_job_desc][]" class="form-control col-md-7 col-xs-12"/></td>' +
    //         '<td><input autocomplete="off" type="text" name="newRefrubishjob[newcost][]" class="decimalOnly form-control col-md-7 col-xs-12"/></td>' +
    //         '<td><input autocomplete="off" type="text" name="newRefrubishjob[desc][]" class="form-control col-md-7 col-xs-12"/></td></tr>';
    ...
    $(document).on('change', '.referal_type', function (e) {
        // alert(999);
        ...
        if (selected == 5) {//Rd customer
            $("#referal_details2").show(); $("#referal_customer_name").prop('required', true); $("#customer_phone").prop('required', true);
            $("#referal_details").hide(); $("#referal_phone1").prop('required', false); $("#referalRdStaffs").prop('required', false);
            $("#referal_details3").hide(); $("#referal_phone3").prop('required', false); $("#referal_name3").prop('required', false);
            //referal_enq_cus_id
        } else if (selected == 4) {//Rd Staff
           // alert(selected);
            var id = $(this).val();
            //var url = site_url + "registration_1/getAllRdStaffs";
            var url = site_url + "registration/getAllRdStaffs";
            $.ajax({
                type: 'get',
                url: url,
                dataType: 'json',
            ...
            $("#referal_details2").hide(); $("#customer_phone").prop('required', false); $("#referal_customer_name").prop('required', false);
            $("#referal_details").show(); $("#referal_phone1").prop('required', true); $("#referalRdStaffs").prop('required', true);
            $("#referal_details3").hide(); $("#referal_phone3").prop('required', false); $("#referal_name3").prop('required', false);


        } else if (selected == '') {//Rd Staff
            // alert(selected);
            $("#referal_details2").hide(); $("#customer_phone").prop('required', false); $("#referal_customer_name").prop('required', false);
            $("#referal_details").hide(); $("#referal_phone1").prop('required', false); $("#referalRdStaffs").prop('required', false);
            $("#referal_details3").hide(); $("#referal_phone3").prop('required', false); $("#referal_name3").prop('required', false);
            ...
            $("#referal_details2").hide(); $("#customer_phone").prop('required', false); $("#referal_customer_name").prop('required', false);
            $("#referal_details").hide(); $("#referal_phone1").prop('required', false); $("#referalRdStaffs").prop('required', false);
            $("#referal_details3").show(); $("#referal_phone3").prop('required', true); $("#referal_name3").prop('required', true);
        }


    });
    $(document).on('change', '.staff_select', function () {
        var phn = $(".staff_select option:selected").attr('data-phone');
        //referal_phone1
        $("#referal_phone1").val(phn);
        //alert(phn);
        //staff_select   
    });
    $(document).on('submit', ".update-refurb-status", function (e) {
       ...
       // alert();
        ...
        $.ajax({
            ...
            success: function (resp) {
                ...
                $("#update-refurb-status").trigger("reset");
                $('.tdriveSearchList')[0].sumo.unSelectAll();
    ...
    $(document).on('keyup', '.customer_phone', function () {
        ...
        var phoneno = $(this).val();
        var url = $(this).data('url');
        $('.customer_phone_msg').html(" ");
        $("#referal_customer_name").val("");
        $("#referal_enq_cus_id").val("");
        //          $('.vreg_cust_phone_msg').html("");
        //          $('.divRegisterHistory').html("");
        if (phoneno.length >= 10 && url !== '') {
            ...
            $.ajax({
                ...
                success: function (resp) {
                    if (resp.status == 'success') {
                        $("#referal_customer_name").val(resp.customer_name);
                        $("#referal_enq_cus_id").val(resp.this_customer_enq_id);
                        // alert(555);
                    } else {
                        $('.customer_phone_msg').html(resp.msg);
                        $("#referal_customer_name").val("");
                        $("#referal_enq_cus_id").val("");
    ...
    $('.cmbRole').on('change', function () {//jsk
        var url = $(this).attr('data-url');
        $('.chkPermissions').prop("checked", false);
        $.ajax({
            type: 'post',
    ...
    $(document).on('keyup', '#val_cust_phone', function () {//jsk
        //          registerAssociation();
        var phoneno = $(this).val();
        //alert(phoneno);
        var url = $(this).data('url');
        ...
        if (phoneno.length >= 10 && url !== '') {
             $.ajax({
                  ...
                  data: {'phoneNo': phoneno},
                  success: function (resp) {
                       $('#ajax_enq_vw').html(resp.enq_data);
                       var formUrl = resp.form;
                       $('#frm_tag').data('url', formUrl);
   
                       var phn = $("#val_cust_phone").val();
                       $("#val_cust_phone").focus();
                       $("#val_cust_phone").val('');
                       $("#val_cust_phone").val(phn);
                       // alert(resp);
   ...
   //                    $('.vreg_cust_phone_msg').html(resp.msg);
   //                    $('.divRegisterHistory').html(resp.regHistory);
   //                    $('#divRegisterHistory_form').html(resp.reghistory_form);
   //                              if (resp.usr_id !== '') {
   //                              }
   //                         }
                  }
    ...
    $(document).on('click', '.tagFollowUp', function () {
        ...
        $.ajax({
            ...
            success: function (resp) {
                ...
                loadFollowupDatePicker();
                $('.divLoading').hide();
                $(".exampleModal").modal();
            }
        });
    });
    $(document).on('change', '.cmbFollowStatus', function () {
          var ststus = $(this).val();
          var option = $('option:selected', this).attr('data-per-day');
          $('.divFollowupDate').html();
          if (option == 0) {
               $('.divFollowupDate').html($('.divNewFollowDateH').html());
          } else if (option == 1) {
               $('.divFollowupDate').html($('.divNewFollowDateW').html());
          } else if (option == 3) {
               $('.divFollowupDate').html($('.divNewFollowDateC').html());
          }

          loadFollowupDatePicker();
     });
    function loadFollowupDatePicker() {
        var maxDate = moment().add(7, 'day');
        $('.dtpNewFollowDateH').datetimepicker({
                    //format: "DD-MM-YYYY",
                    minDate: moment(),
                    useCurrent: false,
                    maxDate: maxDate
                    ...
                    });Commented followup replaced at the same date of followup*/
                });
        //new Date(new Date().setMonth(new Date().getMonth() + 1))
        $('.dtpNewFollowDateW').datetimepicker({
                    //format: "DD-MM-YYYY",
                minDate: moment(),
                useCurrent: false,
                maxDate: moment().add(15, 'day')
        }).on('dp.change', function (ev) { });
        $('.dtpNewFollowDateC').datetimepicker({
                    //format: "DD-MM-YYYY",
                minDate: moment(),
                useCurrent: false,
    ...
    $(document).on('click', '.deleteListItemDataTbl', function () {
